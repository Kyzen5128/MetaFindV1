...
| **R-5** | **I-Design is not on this machine at all, so Table 2 has no input source.** Found by ESSGNN 2026-08-22, verified by Master: the only conda env is `MetaFind` — the `IDesign` env `tools/idesign_generate.py` requires **does not exist**; no I-Design repository is anywhere on disk (`/home/kyzen/upstream/` holds only `ULIP` and `egnn`; `docs/paper/idesign_source/` is the **paper**, not code); and `data/outputs/evaluation_scene_inputs` does not exist. **`tools/idesign_generate.py` cannot execute a single line today.** `n16` reads `evaluation_scene_inputs`, whose only producer is I-Design, so the whole Table 2 chain is upstream-blocked. **Deviations `D-5`, `D-6` and `D-7` all describe modifications to a codebase that is not present** — they are unverifiable in the current tree. **PRIORITY CORRECTED 2026-08-22 — Master ranked this first without reading the registry, and the registry disagrees.** `U-27`'s own `blocking_scope` field: *"Table 2 / scene-composition branch **ONLY**. Data preprocessing, Stage 1 training and Stage 2 training are unaffected … Saying it blocks nothing is as wrong as saying it blocks everything."* And its `do_not_rush` field: *"The 200 prompts should be designed **AFTER** the training protocol settles."* **So R-5 does not outrank the Stage 2 blockers**, and it splits in two: **acquiring** I-Design is pure logistics with an external wait and **no** research decision, and is worth doing early **for a reason unrelated to Table 2 — it is the only thing that makes `D-5`/`D-6`/`D-7` verifiable again**. **Designing** the 200 prompts is a research decision the spec explicitly says must wait. `n14` remains designable now |
