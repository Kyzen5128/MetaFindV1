...
第三步 —— 回報以下八項，然後停下來：
  ...
  4. 已定案、你不准重議的東西有哪些
  5. 你被指派的是 BLOCK.md §6 的里程碑 1（W-1 到 W-8）。
     用你自己的話講一遍你理解的每一項，以及你打算用什麼順序做
  6. 【最重要】比稿的細節提案 —— 我要逐項核准才准動工。至少要涵蓋：
       · 樣本怎麼挑：300-500 筆要用什麼分層？分層的依據是什麼？
         BLOCK.md §11 只寫「stratified」，沒寫依據 —— 這個洞你要填，
         而且要說清楚你的填法為什麼能讓量測有意義
       · 全部 arm 共用同一個樣本，seed 多少，選取規則怎麼重現
       · 樣本產出寫到哪裡。data/outputs/annotations/ 必須維持 0 筆，
         那是全跑專用的。你要怎麼隔離
       · 你要怎麼把 §11 那串指標呈現給我看，讓我能真的做選擇
       · 每個 arm 的 VRAM、每筆秒數、對 45,955 的推估
  7. 需要我決定什麼，你才能開始
  8. 你目前還不確定、或覺得計畫有問題的地方

⚠️ 一個已知的混淆，你必須在第 6 項處理：
   PROMPT_VERSION 5 從來沒有真正跑過。所以這次比稿同時是 v5 的首航。
   v5 本身的 bug 和某個模型的弱點，在數字上長得一模一樣。
   你要說明你打算怎麼把這兩者分開。分不開就明講分不開。

⚠️ 報告完就停。不要寫程式、不要改任何檔案、不要跑任何 GPU job、不要寫 SPEC、
不要載入任何模型。等我逐項核准之後才動工。
```
...
第三步 —— 回報以下八項，然後停下來：
  ...
  5. 你被指派了 BLOCK.md §6 的 W-5、W-6、W-7。用你自己的話講一遍這三題，
     以及你打算怎麼查。特別是：
       · W-5 —— Q-CATEGORY 和 DL-007 是不是同一個問題？
         DL-007 說 LVIS 錨定的設計 2026-08-21 已經被使用者批准了，
         但 Q-CATEGORY 又被列成「從來沒調查過」。兩者不可能都對。
         這題卡住全量標註，先攻它
       · W-6 —— DL-007 自己承認 D0-010 的 §6-§11 是空的，
         也就是 LVIS 錨定是「設計批准」通過的，不是「證據稽核」通過的
       · W-7 —— identity_confirmed 有沒有可能只是橡皮圖章
  6. 在下一次昂貴執行（完整標註 / 全量編碼 / 完整訓練）之前，
     你認為必須先審完什麼
  7. 需要我決定什麼，你才能開始
  8. 你目前還不確定的地方
