...
## 每一段都要標出你在跟誰講話 —— USER 明令 2026-08-22

```
✋ 報告 Kyzen        中文 ＋ ELI5。短句、白話、一行一件事
🤖 給 <角色名>       放進 code block，讓他一鍵複製。裡面用完整專業技術語言
```

**用字固定：`✋ 報告 Kyzen`，不是「給 Kyzen」。** USER 2026-08-22 指定要動詞 ——
那一行要讀起來像「我在向你報告」，不是像貼在盒子上的標籤。

**Kyzen 是靠手動貼上在對話之間傳話的。** 沒有標記他分不出哪段是給他看的報告、
哪段是要他複製出去的。混在一起就是在浪費他的注意力去讀不是寫給他的東西。

- 🤖 的內容**永遠**放在 code block 裡。放外面就不能一鍵複製了。
- ✋ 的內容**永遠不要**用 🤖 的語氣寫。

**標記要往下傳。** 每一個 🤖 區塊，結尾都要叫對方也用同樣兩個標記回報：
✋ 給 Kyzen 的報告、🤖 給其他 Claude 的放 code block。
不然這個規矩只有單向成立，Kyzen 在 Block 視窗裡又得整篇讀完才知道是給誰的。
（六個角色的開場指令已經內建這條，`workflow/roles/README.md`。）

## ✋ 對 Kyzen

**中文 ＋ ELI5。** 短句、短段、白話。路徑、指令、數字要精確。
要他決定時：**最多兩個選項**，講清楚差在哪，並說你會選哪個。

**ELI5 = 沒有大段文字。** 一個重點一行。除非是純數字對照，否則不要用表格。
多欄表格、層層標題、一行裡三個粗體、每項都掛「背景／影響／依據」——
那是把厚重的文章穿上清單的外衣，**不算 ELI5**。

長的、有結構的東西寫進 `workflow/` 檔案，或放進給別的角色的 🤖 貼上區塊。
**不要寫進給 Kyzen 的回覆。**
