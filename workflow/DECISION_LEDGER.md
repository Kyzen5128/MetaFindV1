...
## DL-095 -- ULIP-2 trained FROM SCRATCH on our 80% through ULIP's official main.py (Kyzen 「現在去做」「8020拆分」, 2026-09-04 23:5x)

Kyzen, verbatim: 「python main.py --model ULIP_PointBERT --npoints 8192 --lr 3e-3 --output-dir ./outputs/reproduce_pointbert_8kpts_version_dataset 改成這樣 資料集位置記得改 你可以停掉訓練了不重要」, then 「耖你媽所以你重頭到尾沒有訓練ULIP2?」「Point-BERT不是要重新訓練嗎? CLIP 文字塔、影像塔凍結」「現在去做」「8020拆分」. The DL-093 chain (sel20, lr 1e-4 at epoch 5, holdout mean R@1 0.762) was killed at his order; nothing from it is reported.

What runs (`/home/kyzen/metafind_data_attrs/ulip2_pretrain_run/launch_ulip2_scratch_metafind80_lr3e-3_ep25_20260904.sh`, copies in `tools/ulip2_pretrain/`):
- ULIP's `main.py` UNMODIFIED, model `ULIP2_PointBERT_Colored` (the ULIP-2 builder: OpenCLIP ViT-bigG-14 + Point-BERT depth 18 **from random init**, 10k xyzrgb, `pc_projection` 768→1280, learnable logit_scale clamped ≤ 4.6052), loss `ULIPWithImageLoss` (symmetric P↔I + P↔T, in-batch negatives), AdamW (0.9, 0.98) wd 0.1 with bias/ln/bn exempt, cosine 1-epoch warmup 1e-6 → 3e-3 → 1e-5, AMP, `--lr 3e-3` from the official pretrain script. Single GPU, batch 64 (upstream: 8 × 64 -- RECORDED DIFFERENCE). ULIP-1's `ULIP_PointBERT` (his literal command) needs `slip_base_100ep.pt` and `point_bert_pretrained.pt`, which are not on this machine; the ULIP-2 builder is the one whose released checkpoint we use, so it was chosen (stated to him before launch).
- Data: train = 80% (36,554), served by an adapter registered into ULIP's dataset registry from OUR directory (no upstream file edited): text = the attrs_v1 sentence Stage 1 uses (IMPLEMENTATION CHOICE -- ULIP-2 used the per-view BLIP-2 caption, which we do not have), image = one of the 12 views drawn at random each step (ULIP-2 §3.3), composited on n06's black background then main.py's own train_transform (RandomResizedCrop 224 + ImageNet normalise -- the released loop's choice, recorded), pc = pc_norm(xyz) ++ rgb, no augmentation (the repo's augmentation path is xyz-only).
- Selection: ULIP's own per-epoch zero-shot classification (64 templates, 1,156 LVIS categories) on the 20% (holdout 9,138); best top-1 → `checkpoint_best.pt` (official mechanism, our pool).
- Three wrapper-level changes, none touching upstream files, all recorded: (1) OpenCLIP `requires_grad=False` -- PAPER §3.3 "freeze it" over the released builder's `.eval()` only (without it the optimizer takes 2.5B params: OOM at batch 16); (2) the frozen encoders run under `no_grad`; (3) per-block gradient checkpointing on Point-BERT (memory only; batch 64 × 10k OOMs at 30.4 GiB without it; 19 GiB with).
...
## DL-100 -- Kyzen 「他比較低吧 我沒說錯啊 去測測看」: the frozen-ULIP-2-gallery reading is being tested (2026-09-05 08:1x)

Kyzen's point, from Table 1 itself: no MetaFind fused cell (44.5 / 45.8 / 51.7) exceeds PC-only 75.1, while every fused cell of ours exceeds ours. Agreed as a fact. My position: with the paper's stated formulas (Eq. 1, Eq. 5, Transformer fusion, modality-complete gallery) a query whose text/image identify the same instance can only raise the score; the fused-below-pc pattern needs query text/image that mislead, which is data, not formula. Evidence: the mean-pooling probe (DL-099) with own inputs gives full 100 and with partner inputs 20-69 under the identical formula. Kyzen asked for a test of a formula reading instead; one genuine alternative exists and is now running:

Reading F (sec. 2.4 "gallery encoder is modality-complete and frozen after pretraining"; Fig. 1 "each 3D asset ... pre-encoded independently by ULIP-2 into a fixed vector"): gallery = frozen MEAN of the released ULIP-2 vectors (text, 12-view image, pc; L2-normalised), never trained; only the query Transformer fusion trains (Point-BERT frozen so the gallery pc stays ULIP-2's). Code: `--gallery-fusion mean --freeze-gallery` in stage1.py (recorded in the checkpoint and the arm hash); evaluator and index builder honour the record. Smoke `smoke_frozen_gallery_20260905` trains. Chain `logs/chain_frozen_gallery.sh`: 10 epochs, select on val, then val→val with own and partner queries.

Prediction on record: pc-only falls toward the cross-modal level (released ULIP-2 I→PC 70.4; paper 75.1); with OWN text, T+PC and full stay above pc. If T+PC falls below pc with own text, my formula claim is wrong and I said so in advance.

**DL-100 outcome (15:0x).** Reading F trained (query fusion only, gallery = frozen L2-normalised mean of ULIP-2 text / 12-view image / pc; 10 epochs, best epoch 4, val mean R@1 0.9032). val→val R@1 %:
own queries      text 52.4 image 88.5 pc 99.4 T+I 93.1 T+PC 99.8 I+PC 99.4 full 99.7
partner queries  text  7.7 image  0.6 pc 99.4 T+I  0.9 T+PC 98.6 I+PC 98.3 full 91.8
paper            13.8 / 11.7 / 75.1 / 17.2 / 44.5 / 45.8 / 51.7
Prediction check: "pc-only falls toward ~70" was WRONG -- the query tower learned to map a pc-only query onto the frozen mean vector (99.4). "With own text, T+PC and full stay above pc" held (99.8 / 99.7 vs 99.4). Reading F moves every cell UP (the frozen gallery carries each asset's own text and image vectors verbatim, so text-only jumps 33.8 → 52.4). It is not the paper's formula either. Every formula variant now tried (Transformer fusion trained 1/3/10 epochs, mean pooling untrained, frozen-gallery query-only, normalised or raw) gives fused ≥ pc whenever the query text/image are the asset's own; fused < pc appears only when they are not. Artifacts: `checkpoints/frozen_gallery_mean_fuseronly_lr1e-4_20260905`, `eval/eval_frozen_gallery_mean_fuseronly_lr1e-4_20260905_{own,partner}`.

**Disk tidy (Kyzen 「幫我把 mnt 裡的資料也整理好」, 2026-09-05 17:1x).** `/mnt/data1/kyzen` reorganised by same-filesystem `mv` (renames only, no bytes copied or deleted) into `raw/` (objaverse-lvis glbs 46,052, procthor-10k, ULIP-2 shards 173 GB, shapenet-55 414 GB, text2shape), `models/` (ULIP-2 ckpt, hf-cache, gemma/Qwen), `archive/` (8/21 migrated outputs, 9/2 pre-11view archive, look archives, camera ckpt, blackbox CSVs, ai2thor, envs), `logs/`. Every old path is a symlink to the new one, so `tools/extract_ulip2_query_feats.py`, `annotate_run.py` defaults, `DATA_DELETED.md`, and every ARCHIVED.md reference still resolve (verified). Index: `/mnt/data1/kyzen/README.md`.

**Kyzen 2026-09-05 17:1x, verbatim 「gemma 我是用哪個? 把其他兩個刪掉 / archive 刪掉 / logs刪掉」.** Annotator verified from the annotations themselves: every sampled record says `annotator_model: /home/kyzen/metafind_out/gemma-4-12B-it` (3,000/3,000). Deleted on /mnt/data1: `models/gemma-4-31B-it-qat-w4a16` (22 GB), `models/Qwen3.8-27B` (56 GB) -- bake-off candidates, re-downloadable; `archive/` entirely (8/21 migrated outputs 3.1 GB, 9/2 pre-11view checkpoints/eval/look 22 GB, look_archive_20260901, MetaFindV1_archive (today's output/look archive), camera ckpt, blackbox CSVs, ai2thor, envs) and its symlinks; `logs/` (rsync/download logs, verify_migration.sh). The pre-11view numbers survive only as recorded in this ledger and `output/look/ARCHIVED.md` notes; they can no longer be re-scored. Disk 1.1 TB → 954 GB used. Kept: `raw/` (objaverse-lvis glbs, procthor-10k, ULIP-2 shards, shapenet-55, text2shape), `models/{ulip2,hf-cache,gemma-4-12B-it}`, `MetaFind/{datasets,models}` symlinks. `/home/kyzen/data` is an empty directory (0 bytes); `MetaFindV1/data` is the symlink to `/home/kyzen/metafind_data`.

**VS Code OOM when opening dataset folders (Kyzen 「解決」).** Cause: `renders/` 92,076 entries, `pointclouds/` 92,104, `embeddings/` 91,384, `annotations/` 45,692; the remote VS Code server watches/indexes everything under an opened folder, and inotify `max_user_watches` is 65,536, so it falls back to polling hundreds of thousands of paths. Fix written: `.vscode/settings.json` (workspace: exclude `data/**`, `output/**`, `graphify-out/**` from watcher and search, `search.followSymlinks false`) and `~/.vscode-server/data/Machine/settings.json` (machine: exclude `metafind_data*`, `metafind_out`, `upstream`, `miniconda3`, `/mnt/data1`). Takes effect after reloading the VS Code window. Raising `fs.inotify.max_user_watches` needs root and was not done.

**`/mnt/data1/kyzen/MetaFind/` deleted (Kyzen 「刪掉啊」, 2026-09-05 22:0x).** It was the 8/21 migration directory from the previous machine, not something created by the tidy. Its real contents had already been moved into `raw/` and `models/`; what remained was symlinks plus two stale HF-cache scraps (`models/CACHEDIR.TAG`, one `.locks` file). No code referenced it (`paths.py:125` mentions it only in a comment about the previous machine). The disk top level is now `raw/`, `models/`, `README.md`, and the three symlinks the code does use (`ulip2_objaverse_lvis`, `shapenet-55`, `text2shape`).

**Disk final layout (Kyzen 「整理成 datasets/ 跟 models/ 其他刪了」, 2026-09-05 22:0x).** `/mnt/data1/kyzen` is now exactly `datasets/` + `models/` + `README.md`. `raw/` renamed to `datasets/`; the three top-level symlinks deleted and the four code references repointed: `tools/extract_ulip2_query_feats.py` → `datasets/ulip2_objaverse_lvis/ULIP-2/objaverse_lvis`, `tools/probes/exp_b2_camera_protocol.py` and `tools/probes/exp_b_text2shape_pure_modality.py` → `datasets/{shapenet-55,text2shape}`; every path verified to resolve. `tools/preflight_arms.py` keeps the two deleted VLM arms but each line is now marked DELETED 2026-09-05 (those arms cannot run).

What is unique vs duplicated (measured): UNIQUE on this disk — `datasets/ulip2_objaverse_lvis` 173 GB (every point cloud we use was extracted from it), `datasets/shapenet-55` 414 GB, `datasets/text2shape` 2.1 GB. DUPLICATED on NVMe — `datasets/objaverse-lvis` 328 GB (46,052 glbs both sides), `datasets/procthor-10k` (byte-identical jsonl), `models/ulip2` (same checkpoint), `models/gemma-4-12B-it` (same 30 files as `/home/kyzen/metafind_out/gemma-4-12B-it`), `models/hf-cache` (72 files NVMe vs 92 here, not file-identical).

**`datasets/shapenet-55` KEPT (Kyzen 「沒關係我想要留」, 2026-09-05 22:1x).** 414 GB, unique to this disk, not used by MetaFind (only ULIP-1 reproduction and `tools/probes/exp_b2_camera_protocol.py` / `exp_b_text2shape_pure_modality.py`). Do not propose deleting it again. Disk stays at 954 GB / 3.6 TB used.

**`/home/kyzen` reorganised (Kyzen 「資料集全部統一放在 datasets，其他全部創一個 metafind 資料夾丟進去」, 2026-09-05 22:1x).** Same-filesystem renames, symlink left at every old path:
- `/home/kyzen/datasets/{objaverse-lvis,procthor-10k}` (moved out of `metafind_data/datasets`, which is now a symlink to it).
- `/home/kyzen/metafind/` now holds `metafind_data`, `metafind_data_attrs`, the four small arm roots (`_tokens` P3, `_randview` P6, `_nonorm` P7, `_shared` P4), `metafind_data_desc` (P5; still used for the `desc_v1` query-text override), `metafind_data_json` (P10/P12; `cat_size` override), and `metafind_out`. Each old top-level name is a symlink into it.
- CORRECTION to an earlier claim of mine: `metafind_out` is NOT stale. `metafind_data/outputs/{renders,pointclouds,embeddings,annotations,checkpoints}` are symlinks INTO it, so it holds the live 92,076 renders and 45,692 annotations. I had listed it as "the most worth clearing" on 2026-09-05 17:0x; that was wrong and would have destroyed the corpus.
- Freed: empty `/home/kyzen/data` removed; `metafind_out/gemma-4-12B-it` (23 GB) replaced by a symlink to `/mnt/data1/kyzen/models/gemma-4-12B-it` after verifying 30 files / 23,951,779,497 bytes identical on both sides. NVMe 600 GB → 578 GB used.
- Verified after the move: ten `paths.py` roots, the P1s checkpoint record's `uri` and `initializers.ulip2.uri`, and the 160 glb shards all resolve; `tests/test_splits.py` + `tests/test_train_stage1.py` pass. Index: `/home/kyzen/README_HOME.md`.

**Symlinks removed, paths rewritten (Kyzen 「我不要看到一堆鏈結 給我收進資料夾」, 2026-09-05 22:3x).** The compatibility symlinks left at `/home/kyzen/metafind_*` are gone. Instead of leaving them, every reference was repointed `/home/kyzen/metafind_` → `/home/kyzen/metafind/metafind_`:
- 141 files rewritten: 78 `*_ckpt.json`, 6 `stage1_run.json`, 6 `variant_ckpts.json`, 5 index records (`gallery_index*.json`, `stage2_gallery_index.json`), 4 gate YAMLs, 21 chain scripts, 22 repo source files (`annotate_run.py`, `render_blender.py`, `semantic_edges_run.py`, `run_retrieval.py`, 12 probes, `fingerprint.py`, `reannotate_overlong.py`, `chain_to_stage1.sh`, `tools/ulip2_pretrain/*`).
- 91 symlinks INSIDE the data roots (each arm's `datasets`, `models`, `outputs/{renders,pointclouds,embeddings,annotations,checkpoints}`) repointed; one dangling `metafind_data/envs → /mnt/data1/kyzen/MetaFind/envs` (deleted with the archive) removed.
- 16 shebangs + `pyvenv.cfg` in `metafind_out/bproc_env` repointed -- a moved virtualenv reports ENOENT on the *script* when its interpreter path is stale, which is what made `tests/test_renders.py` fail.
- `MetaFindV1/data` → `/home/kyzen/metafind/metafind_data`.
- NOT rewritten: the `annotator_model` string inside 45,692 annotation JSONs and the provenance blocks inside eval outputs. They are records of what ran, never opened as paths; the model itself is at `metafind/metafind_out/gemma-4-12B-it` (symlink to /mnt/data1).
- Unrelated fix found by the suite: `save_checkpoint`'s new `gallery_fusion` field used `training["fusion"]`, which raised KeyError for callers passing a partial training dict (8 provenance tests). Now `.get("fusion")`.
- Verified: 12 `paths.py` roots exist, **0 dangling `uri` / `initializers.ulip2.uri` across all 99 record files**, `pytest tests/` **1,080 passed**. Index: `/home/kyzen/README_HOME.md`.

## DL-101 -- Kyzen 「完成分析後你自己去測 交給你了」: the weak-but-same-asset query trio is being tested (2026-09-06 04:2x)

Delegated experiment, chosen by me after `docs/NOTE_20260905_TABLE1_WHAT_IT_MEASURES.md`. Every "fused >= pc" observation so far had the query pc EQUAL to the gallery pc (paired cos 1.00), or a degraded pc paired with the asset's own strong text and image. The untested cell is all three query modalities weak but still the asset's own: text = category+size (`cat_size`, alone 17.2 on val) or a ULIP-2 caption (BLIP / Sketchfab name), image = the asset's own Sketchfab thumbnail (a genuinely different rendering; alone 16.8 on the 41K gallery, DL-09x probe), pc = a SECOND surface sample of the same mesh (QueryPack, paired cos 0.99; alone 63.8 on the 36K gallery) or that sample without colour / half-scanned.

Code: `tools/probes/exp_type_level_query.py` gains `--pc-policies` (reuses `perturb` / `encode_clouds` from `exp_query_pc_observation.py` and the QueryPack). Chain `logs/chain_thumbnail_query_val.sh`: P1s and s1_scratchbb, val -> val (protocol C, diagnostic, no sealed read), policies canonical / resample / nocolor / half. Outputs `output/look/exp_type_level_query_{P1s,scratchbb}_val.json`.

Prediction on record (before the numbers): with `resample`, pc-only lands around 80-90 on the 4,569 gallery; T+PC and full stay >= pc because the trained fusion weights the pc token (the DL-098/099 mechanism); I+PC with the thumbnail lands near pc. If T+PC or full falls clearly (>5 points) BELOW pc with weak-but-own text and image and a non-identical own cloud, then the "fused >= pc whenever the observations are the asset's own" statement of DL-100 is too strong and the paper's ordering is reachable without partner queries; I will say so.

**Ops, 2026-09-06 04:2x-04:5x (found while launching DL-101; all three would have hit any GPU run after the 9/5 move).**
1. `~/.cache/huggingface` was a symlink (made 2026-09-02) to `/home/kyzen/metafind_data/models/hf-cache`, the OLD root. huggingface_hub followed the dangling link, re-created `/home/kyzen/metafind_data/models/hf-cache/` and began re-downloading ViT-bigG-14 (9.5 GB) from the Hub, twice (5.2 GB landed). Cause: `paths.setup_env()` sets HF_HOME, but a probe that never calls it falls back to `~/.cache/huggingface`; the vendored `ULIP_models.py:354` `create_model_and_transforms(..., pretrained='laion2b_s39b_b160k')` is what downloads. Fix: symlink repointed to `/home/kyzen/metafind/metafind_data/models/hf-cache` (the real 14 GB cache, bigG blob present), the partial tree (8 files, all dated today) removed, `/home/kyzen` top level clean again.
2. `metafind_data/outputs/_probe/query_pack/query_pack.json` carried two `array` paths under `/home/kyzen/metafind_data_desc/...` (missed by the 9/5 rewrite because `_probe/` was not in the record list). Rewritten to the new root; original kept as `query_pack.json.bak_20260906_oldpaths`; all four arrays verified to exist.
3. Every embedding sidecar in the attrs / desc / json roots (3 x 45,692) had `embedding_uri` under the old root. `encode_text_image.is_complete` requires that path to exist, so a future n06 run in any of those roots would have judged all 45,692 sidecars incomplete and silently re-encoded the corpus (~4 GPU-hours each). Rewritten in place (137,076 files, 3 s), only where the new path exists (137,076 / 137,076). The main root's sidecars (in `metafind_out/embeddings`) were already clean.
Still carrying old-root strings, deliberately untouched (never opened as paths): `annotations/*.json` `annotator_model`, `stage1_hyperparameters.json` `uri` (self-reference; only its `sha256` is compared), `manifest/assets.jsonl` + `captions.jsonl` (read for uid / admitted / render_anomaly only), `reannotation_v9*.json`, `bakeoff/*/annotations_index.jsonl`, `logs/quarantine_*.jsonl`, eval provenance blocks.
Also: `pkill -f <chain name>` and `pgrep -f` matched this shell's own command line twice more today (exit 144). Kill by PID from `ps -eo pid,cmd | grep "[c]hain_..."` only.

**Kyzen 2026-09-06 04:3x, three orders, verbatim:** 「我現在給你設定目標在完成table 1之前不准停下」「你也可以嘗試先完成stage2然後再回來測是不是也要完成stage 2 才會符合table 1 自己判斷」「接下來我將 全程不參與 所以你自己想辦法」. From here every choice is mine and is recorded as IMPLEMENTATION CHOICE / DEVIATION with its reason; nothing is escalated.

**On "is Stage 2 needed for Table 1" (PAPER FACT, `3experiments.tex` §3.2, read 2026-09-05):** "A practical mitigation is to maintain two fusion heads ... Using the Stage-1 head reproduces the 'w/o ESSGNN' numbers (omitted for brevity). In our reported results, we instead explore a single shared head by freezing both encoders in Stage-2, updating only ESSGNN and the fusion, and applying stochastic scene dropout (30%)." So Table 1 row 1 (w/o ESSGNN) is the Stage 1 head, and row 2 (w/ ESSGNN) is the Stage 2 shared head. Stage 2 is not what makes row 1; it is what makes row 2. Both rows are needed for a complete Table 1. Objaverse assets have no scene, so row 2 is scored with the layout term absent (INFERENCE: the paper does not say what ESSGNN receives for a lone asset; scene dropout trains exactly this case).
Chain `logs/chain_stage2_mainline_20260906.sh` (queued behind DL-101): (1) the scratchbb Stage 2 head scored on val->val with `exp_type_level_query.py --stage2-state` (new flag: swaps in the Stage 2 `query.fusion.*` weights, 25 tensors; mask tokens stay Stage 1's because `query_modality_masking none` never trained them; parent sha checked); (2) scratchbb Stage 2 records repointed to the arm copy (sha-equal) before the shared `stage2_full.pt` / `stage2_gallery_index.json` are rewritten; (3) P1s n11b index; (4) P1s Stage 2 with `stage2_hyperparameters_ft_lr5e-5.json` (lr 5e-5, 1 epoch, scene dropout 0.3, τ 0.5, bidirectional), `--query-modality-masking none`, over ALL ProcTHOR train houses (IMPLEMENTATION CHOICE: paper says ESSGNN is trained on >10,000 houses; the S2C/S2D/S2_scratchbb arms used 1,500 for time; ~1 h here) -> `stage2_arms/S2_P1s_none_ft5e-5_allhouses_20260906`; (5) ProcTHOR S1/S2-off/S2-on probe; (6) the P1s Stage 2 head on val->val.

**DL-101 outcome, val -> val, P1s (Stage 1 head), R@1 % (05:0x).** Parity rows reproduce the 9/5 sweep exactly (own 33.8/65.7/97.9/85.9/99.7/98.5/99.8; partner 5.5/1.2/97.9/1.2/94.4/96.1/86.1), so the new code scores what the evaluator scores.
query cloud = second surface sample of the same mesh (paired cos 0.996): pc 97.8 (canonical 97.9). The fine-tuned Point-BERT does not see the resampling.
weak own trio, cloud resampled:   cat_size | thumbnail   17.2 / 57.0 / 97.8 / 63.0 / 97.9 / 98.1 / 98.2
                                  BLIP cap | thumbnail   13.1 / 57.0 / 97.8 / 47.6 / 94.7 / 98.1 / 94.7
                                  u2 name  | thumbnail    6.8 / 57.0 / 97.8 / 45.5 / 94.0 / 98.1 / 95.6
same, cloud canonical:            cat_size | thumbnail   17.2 / 57.0 / 97.9 / 63.0 / 98.2 / 98.3 / 98.4
own cloud without colour (cos 0.872): pc 35.1; own text+view T+PC 45.3 / I+PC 43.7 / full 55.7; cat_size|thumbnail 41.9 / 46.2 / 52.8
own cloud half-scanned (cos 0.718):   pc 19.0; own text+view T+PC 39.7 / I+PC 29.3 / full 51.1; cat_size|thumbnail 34.6 / 32.0 / 46.9
paper w/o ESSGNN                                         13.8 / 11.7 / 75.1 / 17.2 / 44.5 / 45.8 / 51.7
Prediction check: held. With every query modality a weaker second observation of the SAME asset, the fused cells stay at or above pc; the only sign flips are 3 points (BLIP caption 13.1 alone pulls T+PC to 94.7 against pc 97.8; Sketchfab name 6.8 to 94.0). Weak own text does hurt, by 3 points, not by the paper's 30. The thumbnail (image alone 57.0 on this 4,569 gallery; 16.8 on the 41K gallery on 9/4) never hurts I+PC. Degrading the cloud itself (no colour, half scan) drops pc below the singles and the fused cells rise above pc -- the paper's ordering inverted. The DL-100 statement stands and is now measured on the last untested cell: fused >= pc for any same-asset query, exact cloud or not; fused < pc needs query text/image that do not identify the asset (partner). Artifacts: `output/look/exp_type_level_query_P1s_val.json` (scratchbb running).

**Scratchbb Stage 2 shared head on the Objaverse Table 1 protocol (val -> val, layout absent; DL-101 chain step 1, 04:50).** own 21.5 / 43.3 / 83.5 / 63.2 / 94.7 / 90.7 / 94.9 (Stage 1 head: 32.6 / 60.9 / 88.4 / 86.4 / 99.7 / 97.4 / 99.9); partner 3.0 / 1.1 / 83.5 / 1.4 / 76.5 / 62.3 / 57.4; cat_size | thumbnail 10.7 / 30.6 / 83.5 / 42.7 / 89.4 / 87.6 / 89.9. The Stage 2 fine-tune on 1,500 ProcTHOR houses moved every cell down (text -11, image -18, pc -5, T+I -23) -- the paper's "feature-attribution mismatch" measured on Objaverse: paper w/ vs w/o is -2.5 / -1.2 / -11.9 / -1.3 / -3.3 / -3.8 / -3.5. Ordering unchanged: fused >= pc with own queries. So Stage 2 does not produce row 1's shape either; it produces row 2's drop. Artifact: `output/look/exp_table1_stage2head_scratchbb_val.json`.

**DL-101 outcome, val -> val, s1_scratchbb (Stage 1 head), R@1 % (05:0x).** Parity reproduced (own 32.6/60.9/88.4/86.4/99.7/97.4/99.9). Second surface sample: paired cos 0.993, pc 87.4 (canonical 88.4). Weak own trio: cat_size | thumbnail | resample 14.0 / 42.4 / 87.4 / 62.1 / 97.1 / 96.1 / 97.9; BLIP | thumbnail | resample 6.8 / 42.4 / 87.4 / 35.1 / 86.7 / 96.1 / 91.3 (T+PC below pc by 0.7 with a 6.8 text; full above). Partner 4.5 / 0.8 / 88.4 / 0.8 / 85.0 / 78.9 / 73.3. Degraded own cloud: no colour pc 8.5 (this backbone leans on colour far more than the released one: cos 0.665 vs 0.872), half scan 21.4; in both cases the fused cells climb far above pc (own text+view full 36.6 / 80.9). Same reading as P1s: the from-scratch backbone's weaker pc fingerprint (88 vs 98) does not change the ordering. Artifact: `output/look/exp_type_level_query_scratchbb_val.json`.

**Rung 0 x weak own trio (released ULIP-2, no trained fusion; val -> val; `tools/probes/exp_mean_pool_weak_trio.py`, 05:07).** Gallery = pooled unit vectors (attrs text, 12-view image, canonical pc); query = pooled unit vectors of the present modalities. `mean` pooling: cat_size | thumbnail | resample 49.8 / 86.0 / 99.5 / 89.1 / 99.6 / 99.5 / 99.6; BLIP | thumbnail | resample 39.5 / 86.0 / 99.5 / 80.3 / 99.4 / 99.5 / 99.2; partner 5.4 / 0.2 / 99.5 / 0.1 / 98.2 / 97.7 / 67.3. `raw` pooling (text/image norms outweigh pc): partner 4.8 / 0.2 / 99.5 / 0.1 / 86.8 / 52.0 / 20.3 (the DL-099 baseline-shape row, reproduced); weak own trio 52.0 / 85.1 / 99.2 / 89.5 / 99.5 / 98.7 / 99.2. So an untrained combiner with weak same-asset observations still gives fused >= pc, and text-only is HIGHER than with the trained tower (49.8 vs 17.2) because the pooled gallery vector carries the asset's own text component and the category+size string shares its numbers with the attrs sentence. Closing the val side of DL-101: trained (Stage 1 head, Stage 2 head) or untrained (mean / raw pooling), exact or resampled cloud, own or weak-own text and image -- every same-asset construction gives fused >= pc. The paper's ordering appears only with partner text/image. Artifact `output/look/exp_mean_pool_weak_trio_val.json`. BLIP caption missing count printed in the log (fell back to the Sketchfab name feature).

**P1s Stage 2 done (06:15; `stage2_arms/S2_P1s_none_ft5e-5_allhouses_20260906`; 9,600 train houses = our 80% of ProcTHOR-10K's 12,000, ~10,200 steps, 66 min; lr 5e-5 cosine, 1 epoch, scene dropout 0.3, bidirectional, `query_modality_masking none`, gallery frozen).** ProcTHOR probe, 300 test houses, leave-one-out queries against the 1,439-asset gallery: S1 head 86.7 / 99.9; S2-off (Stage 2 fusion, no layout) 34.3 / 79.5; S2-on (+ lambda * ESSGNN) 35.1 / 79.3. Same shape as the 1,500-house arms (scratchbb 81.8 -> 44.3 -> 43.9; S2D 82.4 -> 36.8 -> 32.8): the fine-tune costs the layout-free head ~50 points on its own training distribution and the layout term gives back under one point. More houses did not change that. Artifact `output/look/exp_stage2_procthor_retrieval_P1s_allhouses_20260906.json`.

**P1s Stage 2 shared head on the Objaverse Table 1 protocol (val -> val, layout absent, 06:19) -- the first same-asset construction with fused cells AT or BELOW pc.**
own text + own view + own cloud      23.2 / 39.8 / 74.9 / 54.9 / 77.3 / 77.3 / 79.0
cat_size | own thumbnail | own cloud 11.4 / 32.9 / 74.9 / 34.1 / 68.8 / 72.3 / 68.7
own text | partner thumbnail         23.2 /  2.3 / 74.9 /  7.7 / 77.3 / 55.2 / 61.7
partner text + partner view           5.3 /  1.6 / 74.9 /  2.5 / 66.8 / 60.8 / 46.3
paper w/o ESSGNN (Stage 1 head)      13.8 / 11.7 / 75.1 / 17.2 / 44.5 / 45.8 / 51.7
paper w/ ESSGNN (Stage 2 head)       11.3 / 10.5 / 63.2 / 15.9 / 41.2 / 42.0 / 48.2
Stage 1 head, same queries:          33.8 / 65.7 / 97.9 / 85.9 / 99.7 / 98.5 / 99.8  and  17.2 / 57.0 / 97.9 / 63.0 / 98.2 / 98.3 / 98.4
Reading. The 9,600-house Stage 2 fine-tune moves the query fusion off the gallery fusion far enough that a pc-only query no longer lands on the gallery vector: pc 97.9 -> 74.9 (the paper's Stage 1 head sits at 75.1). Once the pc identity is gone, a weak same-asset text (category+size, 11.4 alone) pulls T+PC to 68.8 and full to 68.7, BELOW pc, and the thumbnail pulls I+PC to 72.3, below pc: the paper's ordering pc > I+PC ~ T+PC ~ full > T+I > singles, from the asset's OWN observations for the first time. With own text and own view the fused cells sit 2-4 points above pc (77-79). The scratchbb Stage 2 head, trained on 1,500 houses (~1,600 steps), drifted less (pc 83.5) and kept fused >= pc; drift scales with Stage 2 length. So the DL-100/101 statement needs its scope fixed: "fused >= pc for any same-asset query" held for every head whose pc-only match was near-exact (>= 88); it does not hold once the query head is pushed off the gallery head. What the paper's Stage 1 head did to sit at 75.1 before any Stage 2 remains UNKNOWN (paper: "our dual-tower framework introduces more cross-modality retrieval, which results in lower accuracy under PC only").
Which paper row is which (settled from the full paragraph, `3experiments.tex` §3.2): "w/ ESSGNN" is the Stage 2 shared head evaluated on Objaverse-LVIS "which lacks layout and disables ESSGNN" -- i.e. exactly the layout-absent scoring done here; the drop from row 1 is "residual attribution drift". "w/o ESSGNN" is the Stage 1 head ("Using the Stage-1 head reproduces the w/o ESSGNN numbers"). Row 2 of our Table 1 is therefore this head; the holdout numbers follow in `chain_table1_final_20260906.sh`.

## DL-102 -- Table 1 completed on the 20% holdout: two backbones x two heads x three query constructions (2026-09-06 06:45)

Kyzen's order (04:3x): 「完成 table 1 之前不准停下」, 「全程不參與」. Chain `logs/chain_table1_final_20260906.sh`, holdout 9,138 -> holdout 9,138, `tools/probes/exp_type_level_query.py --query-split holdout --gallery-split holdout --pc-policies canonical,resample`. Parity: the P1s Stage 1 head's own row (25.0 / 57.3 / 96.7 / 80.2 / 99.5 / 97.5 / 99.6) matches the 9/4 sealed A20 run (25.0 / 57.2 / 96.7 / 80.4 / 99.5 / 97.6 / 99.6, query = test half only); the scratchbb Stage 1 head's own and partner rows match the 9/5 official holdout run to the decimal. So the probe scores what the evaluator scores.

Mainline P1s, R@1 / R@5 (%):
                         text          image         pc            T+I           T+PC          I+PC          full
paper w/o ESSGNN         13.8 / 23.1   11.7 / 19.2   75.1 / 78.0   17.2 / 21.8   44.5 / 71.3   45.8 / 73.1   51.7 / 76.5
S1 head, own             25.0 / 54.9   57.3 / 80.8   96.7 / 99.6   80.2 / 95.2   99.5 / 100.0  97.5 / 99.8   99.6 / 100.0
S1 head, weak own        12.1 / 32.9   47.1 / 76.5   96.7 / 99.6   53.7 / 81.9   97.1 / 99.9   97.1 / 99.8   97.1 / 99.9
S1 head, partner          3.6 / 16.3    0.8 /  9.2   96.7 / 99.6    1.0 / 13.7   92.3 / 99.1   94.3 / 99.4   81.7 / 96.0
paper w/ ESSGNN          11.3 / 21.5   10.5 / 15.9   63.2 / 66.5   15.9 / 20.3   41.2 / 68.8   42.0 / 70.4   48.2 / 74.9
S2 head, own             16.4 / 41.5   31.5 / 59.5   66.8 / 90.4   46.0 / 76.0   69.4 / 91.4   69.6 / 91.7   71.0 / 92.3
S2 head, weak own         7.0 / 22.3   24.5 / 50.2   66.8 / 90.4   25.4 / 52.8   59.5 / 85.6   63.4 / 88.3   59.6 / 85.7
S2 head, partner          3.3 / 13.5    1.1 /  7.2   66.8 / 90.4    1.6 / 11.4   58.4 / 85.4   51.9 / 81.4   38.6 / 70.3
weak own = category+size text (Figure 1 form), the asset's own Sketchfab thumbnail, its own cloud. partner = same-category other asset's attrs text and view, own cloud. Resampled-cloud variants differ by <= 1 point (paired cos 0.996). Full tables incl. BLIP-caption rows: `docs/TABLE1_REPORT_20260906_v3.md` §4.2; JSON `output/look/table1_final_*_holdout.json`.
Second backbone (s1_scratchbb; Stage 2 on 1,500 houses): S1 head own 24.0 / 52.3 / 84.4 / 81.4 / 99.5 / 96.0 / 99.7; S2 head own 14.8 / 33.8 / 76.9 / 52.8 / 91.5 / 85.9 / 91.5; S2 head weak own 6.7 / 22.1 / 76.9 / 32.9 / 84.0 / 82.3 / 84.8 (R@1).

Findings.
1. Row 1 (Stage 1 head): every cell far above the paper, fused >= pc under every same-asset construction (val exhausted these: own / cat_size / BLIP / Sketchfab-name text; own view / own thumbnail; own / resampled cloud; trained Transformer or untrained mean pooling). The paper's ordering needs partner text/image, which kills the single-modality cells. Both layers of the gap (query observation source; pc correspondence 75 vs 97) are outside what more runs can settle.
2. Row 2 (Stage 2 shared head, layout absent -- exactly how the paper scores it, §3.2 "lacks layout and disables ESSGNN"): the 9,600-house fine-tune moves the query head off the gallery head, pc 96.7 -> 66.8 (paper 63.2). With weak own observations the paper's ordering appears from the asset's OWN observations for the first time: pc 66.8 > I+PC 63.4 > full 59.6 ~ T+PC 59.5 > T+I 25.4 ~ image 24.5 > text 7.0. With own text and view the fused cells sit 2-4 points above pc. Still off: image 31.5 vs 10.5, T+I 46.0 vs 15.9, fused cells +10..+30, and R@5 pc 90.4 vs 66.5 (the paper's pc misses are catastrophic, ours are near misses).
3. Drift scales with Stage 2 length: 1,500 houses (scratchbb) pc 76.9 and fused >= pc; 9,600 houses (P1s) pc 66.8 and fused <= pc. Consistent with the paper's "residual attribution drift"; but the paper's ROW 1 already shows this behaviour before any Stage 2, and why its Stage 1 head sits at 75 is UNKNOWN.
4. Kyzen's 「先完成 stage 2 再回來測」: the paper's row 1 is not a Stage 2 product (PAPER FACT, §1 of the report), but the paper's SHAPE appears here only after Stage 2. Half right, and the useful half.

Decisions (mine, Kyzen absent): own observations is the primary protocol row (domain convention; Ex-MCR); weak own and partner are reported as labelled alternates; Stage 2 for the mainline uses all 9,600 train houses (paper >10,000; the 1,500-house arms were a time bound); selection pools stay as they were (P1s val, scratchbb holdout) and are stated; no row is called a reproduction. Status: protocol IMPLEMENTED / EXECUTABLE / BEHAVIOR-VERIFIED; numbers vs paper UNVERIFIED; query construction IMPLEMENTATION CHOICE; corpus 45,692 vs 48K DEVIATION. Questions only the authors can answer are listed in the report §5.6; no email sent (outward-facing, not authorised).

**Kyzen 07:0x-07:2x, verbatim:** 「2.（補測空圖版）然後 補完報完後 你再重新處理資料集吧 把之前的錯誤都修正 這次資料集處理方法 直接採用 論文所描述的 擬定完流程後 二次確認 除了LLM 其他一律按照論文要求 不清楚就找資料」「一樣全程交給你負責 全線都給你 直到完成」. The dataset rebuild is DL-103 below.

**Lone-node ESSGNN reading of row 2 (Kyzen's 「2.」; `tools/probes/exp_stage2_head_lone_node.py`; P1s Stage 2 head, holdout 9,138 -> 9,138, 07:40).** Each Objaverse asset pushed through ESSGNN as a one-node, zero-edge graph (node text "a {lvis_category}", the ProcTHOR node construction; E = 0 makes the EGCL layers the identity, normalised-sum pooling gives |e_layout| = 1), lambda = 21.3 after training, |Fusion(full query)| = 386, so the layout term is 5.5% of the query. R@1: own 16.4/31.5/66.8/46.0/69.4/69.6/71.0 (absent) vs 16.2/30.9/66.1/45.2/68.5/68.8/70.1 (lone node); weak own 7.0/24.5/66.8/25.4/59.5/63.4/59.6 vs 6.9/24.0/66.1/25.0/58.7/62.6/58.7. Every cell moves 0.2-1.0 down; ordering unchanged. The two readings of "disables ESSGNN" give the same Table 1 within one point; the layout-absent reading stays the reported one (the paper's own words). Artifact `output/look/exp_stage2_head_lone_node_P1s_holdout.json`.

**Code search (07:5x).** The poster QR (`docs/reference/metafind_neurips2025_poster.png`) decodes to https://scanned.page/HA6hkN, which now serves only the QR vendor's landing page; OpenReview sits behind a bot wall; the NeurIPS virtual page links the poster PNG and OpenReview only; web search finds no repository. No official code or data release is reachable, so every unspecified preprocessing detail stays IMPLEMENTATION CHOICE.

## DL-103 -- Dataset rebuilt from the paper's text (Kyzen 07:0x 「重新處理資料集 ... 直接採用論文所描述的 ... 除了LLM 其他一律按照論文要求 不清楚就找資料」「全線都給你 直到完成」), 2026-09-06 08:1x

Plan: `docs/DATASET_REBUILD_PLAN_20260906.md` (every paper sentence about data quoted; each step classified; Kyzen's 9/2 rulings DL-077/078/079 reused). New root `/home/kyzen/metafind/metafind_data_paper/` (renders -> `metafind_out/renders_v7`, annotations -> `annotations_v10`, embeddings -> `embeddings_v10`; point clouds shared; ProcTHOR artefacts linked to the main root until R6 regenerates them). Nothing in the old roots is touched.
What changes against the v6 corpus: 11 views (one 20-degree orbit, equal azimuths, perspective, 512 px, OpenShape's Blender script, renderer v7 -- Kyzen's 甲 of DL-077 Q2 / DL-079 Q3, never executed until now); the VLM sees all 11 views and returns the Figure 2 schema with VLM-estimated width/length/height (paper: GPT-4o supplies "size dimensions"; the mesh-proportion derivation is dropped from the record), one description (no CLIP-ranked candidates), synset asked of the model; gallery text = the Figure 2 JSON string (`figure2_json`; INFERENCE from Figure 2 / poster; CLIP truncation at 77 tokens is recorded, not avoided); image = mean of the 11 views on both towers (DL-079 Q5); 80/20 split, seed 20260816, select and report on the 20%; the 311 quarantined assets retried with a per-attempt seed (old bug: seed bound to uid, so a retry was a no-op); 21 manual exclusions kept; ProcTHOR unified to the same render protocol with gemma node captions (DL-077 Q10/Q11). Kept as IMPLEMENTATION CHOICE with reasons: the LVIS label is still shown to the VLM (paper silent on withholding it; blind gemma misidentified 28% in v7's measurement -- DL-007's "DEVIATION" reclassified), gemma instead of GPT-4o (Kyzen's one exception), ULIP-2 official 10k clouds (paper silent), kNN k=8 adjacency.
Second check: (1) plan re-read against the quoted sentences; (2) renderer v7 smoke on 3 assets under the new root: 11 views, `orbit_elevation_deg 20.0`, perspective, 512, `renderer_version 7`, sidecars written, images inspected; (3) read-only reviews of the plan dispatched to the ULIP2 and ESSGNN reviewer agents, findings to be folded in before R2 (annotation) starts; (4) each long job logged here before launch.
R1 launched 08:1x: `python -m metafind.data.renders --workers 8` under `METAFIND_DATA=metafind_data_paper`, 46,049 assets to go, ~3 s/asset -> about 1.7-2 days; log `metafind_data_paper/outputs/logs/r1_render_v7.log`.

**R1 done (2026-09-06 08:15 -> 14:14, 6 h at ~120 assets/min with 8 Blender processes; the 1.7-2 day estimate assumed the per-asset time was not overlapped).** 46,052 in manifest; 46,012 rendered under renderer v7 (11 views, one 20-degree orbit, perspective, 512 px, transparent RGBA), `renders_index.jsonl` 46,012; 40 quarantined (33 `DETERMINISTIC_INPUT` "every view drew nothing", 7 `UNKNOWN`); 34 of the 40 were also quarantined in the v6 run, 6 are new. Store `metafind_out/renders_v7/` (about 70 GB). The two plan reviewers were re-dispatched at 15:05 (their first run died on the 08:2x rate limit).
R2 code: `metafind/data/annotate_v10.py` (one-call Figure 2 prompt; VLM-estimated width/length/height; volume computed; synset asked, lookup fallback; one description; LVIS label shown; 3 attempts, last one sampled with a per-attempt seed) and `annotate_run.py --prompt-mode figure2_v10` (contract id and required-field set swapped through module-level indirection; the v9 path untouched). Unit tests `tests/test_annotate_v10.py`. Smoke on 5 assets launched 15:1x.

**Second check of DL-103 (two read-only reviewers, 15:05-15:25) and what changed.**
ULIP2 reviewer: BLOCKER 1 · MAJOR 2 · MINOR 6 · INFO 4. ESSGNN reviewer: MAJOR 1 · MINOR 5 · INFO 4. Findings and disposition:
- BLOCKER (encoder). `encode_text_image.refuse_if_overlong` REFUSES text over 77 tokens; the Figure 2 JSON is 135 tokens for the paper's own example and 137-146 on the smoke records, so R4 would have quarantined the corpus. Fixed: under `METAFIND_TEXT_TEMPLATE=figure2_json` only, the encoder encodes the text as CLIP's tokenizer does (truncate at 77, EOT re-inserted) and records `text_truncated: true` + the true count; the run summary says TRUNCATED. IMPLEMENTATION CHOICE against a paper silence; consequence recorded: CLIP sees `{"annotations": {"category", "synset", width/length/height, volume, mass, "description": "<first ~15 words>` and never the materials or placement flags. Decision: `figure2_json` stays the mainline text (it is the artifact the paper and poster print); the sentence template `v2_cm` (all fields, fits 77) is run as the alternate text arm on the same corpus so the cost of the literal reading is measured, not assumed.
- MAJOR (formatting). `figure2_json_string` emitted `30.0`/`36000.0`; Figure 2 prints `30`/`36000`. Fixed: integers where whole, 2 dp otherwise (tested).
- MAJOR (contract vs views). v10 hashed its contract on 11 views but never checked the input. Fixed: `annotate_one` quarantines any asset whose render has != 11 views or renderer_version != 7 (`WrongRenderProtocol`); both constants are in the contract.
- MINOR fixed: WordNet existence check on a well-formed model synset (fallback otherwise, source string says why); `--retry-salt` recorded in each record so a re-run over v10's quarantine is not the same three attempts; category "1-4 words"; description minimum length; >6 materials refused (repair) instead of cut; gen-exception quarantine keeps a traceback; `first_error` recorded (the smoke showed every asset needing attempt 2 -- the model wraps the object in `{"annotations": ...}` as Figure 2 does; the validator now unwraps it).
- MINOR (labels, plan §1): ULIP-2 §4.1 renders ONE orbit of 12 (UPSTREAM FACT); OpenShape's three rings are not ULIP-2's layout; 20 degrees / camera_dist 1.2 / 512 px are UNKNOWN upstream; the single orbit against the paper's "orthogonal" + Figure 2's mixed elevations is a Kyzen-ruled DEVIATION (DL-077 Q2), not paper silence. Image aggregation: upstream uses one random view per step and averages only text prompts (normalise-mean-normalise); the plain 11-view mean is DL-079's choice. Plan text corrected.
- INFO: OpenShape details recorded (35 mm lens / 32 mm sensor, bbox-cube x0.8, one 30 kW area light, 32 samples, OptiX ours, upstream alpha-drop vs our black source-over).
- ESSGNN MAJOR (graph unit). Paper: "room-level", "Each room configuration", "single-room indoor scenes" (four sentences); our scene graphs are one per HOUSE with kNN across rooms. To be fixed before R6: per-room graphs (objects assigned to rooms from ProcTHOR's room polygons, children with their parent). Affects Stage 2 only (row 2).
- ESSGNN MINOR: "gallery frozen" -> "both encoders frozen, only ESSGNN + query fusion train" (code already complies); h^(0) = t_i (not Concat(x_i, t_i)) is the documented resolution of the paper's own contradiction with its appendix premise -- named in the plan; ProcTHOR "comprehensive semantic metadata" DOES exist upstream (asset-database.json, ai2thor-object-metadata.json, placement-annotations.json, receptacles.json in the procthor release) -- the 9/2 premise "does not exist" was "not downloaded"; to be fetched and read before deciding node text (DL-077 Q10 gemma captions was ruled on the false premise); the unified ProcTHOR render path is not implemented yet (procthor_modalities.py hardcodes orthographic/white); depth-shell clouds unlike Objaverse clouds (recorded, unchanged).
- ESSGNN INFO: Table 1 row 2 layout-absent is the paper's own wording; lone node contradicts "disables ESSGNN" (kept only as a diagnostic); Stage 2 epochs = 1 pre-registered here and will not be revisited after Table 1; Stage 2 tau follows Stage 1's fixed 0.5.
Tests: 63 pass (`tests/test_annotate_v10.py`, `tests/test_encode_text_image.py`). Smoke b (5 assets, fixed contract `metafind_annot_v10@f0c98e5540218047`) launched 15:3x.

**R2 launched (2026-09-06 15:5x).** Smoke a/b: every asset needed attempt 2 -- the cached-prefix path (`SharedViewPrefix`) returned malformed JSON on the first call of every asset while plain `Annotator.generate` returned valid JSON first time; v10 makes one call per asset, so the prefix cache had nothing to share and is not used (smoke c: 3/3 on attempt 1). Command `logs/r2_annotate_v10.sh` (= `annotate_run --prompt-mode figure2_v10`, contract `metafind_annot_v10@f0c98e5540218047`), 46,012 rendered assets, 8 already done; output `metafind_out/annotations_v10/`. The chain `logs/chain_paper_pipeline_20260906.sh` is queued on `=== R2 DONE`: 21 manual exclusions -> n05b (figure2_json, mean of 11 views) -> n06 -> n09 (80/20, seed 20260816) -> Stage 1 (same_record, same_mean, lr 1e-4, 10 epochs, select on the 20%) -> n11/G4/n12 -> Table 1 row 1 (official evaluator, A20_holdout_vs_holdout + B) -> n11b -> Stage 2 (9,600 houses) -> ProcTHOR probe -> both heads on holdout (own / weak own / partner) -> tables. Room-level scene graphs and the ProcTHOR node text from upstream metadata (second-check findings) are being implemented while R2 runs; if they land before the chain reaches n11b, the chain's Stage 2 steps are re-run on the new graphs.

**R6 prep while R2 runs (15:3x-15:5x).**
- Room-level scene graphs (`scene_graphs.py` BUILDER_VERSION 2; ESSGNN reviewer MAJOR): adjacency kNN inside each room; Stage 2 context = the target's room minus the target (legacy house graphs unchanged so old checkpoints evaluate as before). Built into the new root: 12,000 houses, 1,467 assets, 0 quarantined, every node has a room (3.8 rooms with objects per house). Per house the edge counts barely move (adjacency 299 -> 290, semantic candidates 340 -> 331 on the first 2,000 houses); the material change is the CONTEXT (one room instead of the whole house). Tests added.
- ProcTHOR node / gallery text from upstream metadata (`tools/procthor_metadata_text.py`): `asset-database.json` (bounding box -> cm, materials, objectType) + `placement-annotations.json` (onFloor / onWall / room counts / pickupable) shipped in the cloned procthor repo cover all 1,467 assets. Each asset gets a Figure-2-shaped record (mass null: not in the metadata; description null until gemma captions, R6b), the text tower reads its `figure2_json` string (same construction as the Objaverse gallery text), and the relation-sentence prompt reads a one-sentence `relation_text` (`semantic_edges_run.collect_pairs` prefers it). Written to the new root; the category-only map is kept beside it. This replaces DL-077 Q10's gemma-caption node text as the PRIMARY source (that ruling rested on the false premise that the metadata did not exist); captions are added on top when R6b runs.
- ProcTHOR asset renders unified with the Objaverse protocol (`procthor_modalities.py` BUILDER_VERSION 2, DL-077 Q11 甲, now implemented): constants imported from the LIVE renderer (`render_blender` v7: 11 azimuths, 20 degrees, 512 px) instead of the retired pyrender module; RGB pass is PERSPECTIVE at OpenShape's framing (35 mm lens on a 32 mm sensor = 49.13 degree FOV; camera distance 1.5 x the largest bounding-box extent, which is 1.2 / 0.8 in OpenShape's normalised scene), black skybox (Kyzen 甲-2 "black composite"); depth pass and depth-shell clouds unchanged. Recorded differences: Unity renderer and lighting, opaque black skybox instead of transparent alpha composited on black. Test repointed at the live constants. Smoke on 2 assets launched 15:3x into the new root's own `procthor_modalities/`.
- Ops: `~/.ai2thor` pointed at `/mnt/data1/kyzen/MetaFind/ai2thor`, deleted with the 9/5 archive on Kyzen's order, so AI2-THOR had no Unity build. Re-created as `metafind_out/ai2thor` (NVMe) behind the same dotfile symlink; the 5.0.0 build re-downloads on first use.
- R6a launched 15:4x: unified ProcTHOR renders for all 1,467 assets (`n07b`, BUILDER_VERSION 2) into the new root; smoke: perspective, black skybox, 49.13-degree FOV, depth-shell cloud bbox ratio 1.04.
- Stage 2 protocol resolver records `graph_unit: room` (`resolve_stage2.py`, tests pass). `tools/procthor_captions.py` (R6b): one gemma sentence per ProcTHOR asset from its 11 unified views, merged into the metadata records as Figure 2's `description`. Relation sentences for n08 are TYPE-level (category + placement; 93 distinct) so the semantic-edge LLM calls stay at the ~4K-pair scale instead of per-asset pairs.
- Queue (all background, each gated on the previous log's DONE line): R2 annotation (running, ~2.7 s/asset, ~1.5 days) -> `chain_paper_stage1_20260906.sh` (exclusions, n05b, n06, n09, Stage 1 same_record/same_mean lr 1e-4 10 ep select-on-20%, n11/G4/n12, row 1 on holdout) -> `chain_paper_stage2_20260906.sh` (R6b captions, metadata text, n09b, n08 semantic edges + node embeddings, n09c splits, n11b, Stage 2 9,600 houses 1 epoch, ProcTHOR probe, both heads on holdout, tables). Pre-registered: Stage 2 epochs = 1, not revisited after Table 1.

**Kyzen 17:0x, verbatim:** 「你標註怎麼做有長度限制？」→「你沒限制的話那你不就一堆標註等於沒用」→「每個細節都要注意喔 我可以給你所有權限 但麻煩你 每個細節都要做好」.
He is right and the mainline text changes: under `figure2_json` CLIP saw ~77 of ~140 tokens -- materials, the four placement flags and most of the description never reached the encoder, so two of the four attributes the paper names were annotated for nothing. Decision (mine, under his delegation): the mainline gallery text is `v3_fit` -- the v2_cm sentence (description, category, materials, dimensions, placement) with the description trimmed at a word boundary until the whole string is within 77 true tokens (`resolve_stage1.serialize_fitted` / `fit_to_context`; content-addressed identity `metafind_v3_fit@afcf873ebf5f482e`; contract records the budget and the rule). Every structured field always reaches the encoder; only the description tail can be cut, deterministically; the structured fields alone overflowing is refused, not trimmed. Measured on the first v10 records: 57-64 tokens, nothing trimmed (the v10 prompt asks for ~30 words). The Figure-2 JSON string stays as the LITERAL-READING ARM (a second root, same renders / annotations / clouds, own encoding and Stage 1), run after the mainline so the cost of the literal reading is a measured number. ProcTHOR node / gallery text uses the same fitted sentence (`procthor_object_text.json` `text`; the JSON kept as `text_figure2_json`; max 41 tokens). R2 (annotation) is unaffected: every field is in the record; only the encoder-side serialisation moved. Both queued chains relaunched with `METAFIND_TEXT_TEMPLATE=v3_fit`. Tests: 5 new for the fit.
...
**ESSGNN REVIEWER on DL-104 (14:2x): MAJOR 1, MINOR 5, INFO 6 -- all fixed, commit follows.** MAJOR 1: the declaration was enforced by chain order only; a pre-ruling `stage2_protocol.json` (which is what sits in the paper root until the chain's step 2 rewrites it) defaulted n11b and the trainer back to three modalities silently. Now `asset_modalities` is in `load_stage2_protocols.implemented` (a protocol without it, or with another value, is refused -- this also refuses the attrs root's old protocol, which is intended: fail loud, rebuild), n11b refuses a protocol without the field, and the trainer refuses an index whose `modality_completeness.declared_modalities` differs from the protocol. MINOR 1: `decay_mask_tokens` is live now (only the pc row gets a gradient); the trainer refuses `true` while the tokens train under `none`; the "inert today" comment corrected. MINOR 2: the S2-head probe line now says whether the mask tokens came from Stage 2 (they do). MINOR 3: n09b's summary line no longer says "no point cloud". MINOR 4: `stage2_smoke_seven_checks.py` passes the declaration. MINOR 5: an emptied presence row is repaired with the first DECLARED slot. INFO 5/6: stale prose in `dual_tower.py` and the ProcTHOR probe. Tests: +1 (protocol without the field refused); 1,052 pass on CPU.

**SCA3D repository read (Kyzen 15:0x 「clone 下來看一下有沒有」)** -- cloned to `/home/kyzen/upstream/SCA3D` (3DAgentWorld/SCA3D, 2 commits). No mention of MetaFind, Objaverse, LVIS, ULIP or ProcTHOR anywhere. Its evaluation (`SCA3D/evals/eval.py`) is the Text2Shape protocol on ShapeNet chairs + tables: PointNet++ part features, EMD matching, shape->caption and caption->shape RR@1/5/10 and NDCG@5, gallery = all test shapes after `del_repeat`, queries = each test shape's OWN human captions (several per shape). UPSTREAM FACT. Nothing it says bears on how MetaFind built Table 1 queries; it does show that caption->shape R@1 in the teens is the normal level even with the shape's own captions, so the paper's text-only 13.8 is unsurprising on its own -- the image-only 11.7 and pc-only 75.1 are the cells our own-data construction cannot reach downward.

**Uni3D repository read (Kyzen 15:1x)** -- already at `/home/kyzen/upstream/Uni3D` (baaivision/Uni3D). No mention of MetaFind / ProcTHOR / ESSGNN. The only quantitative evaluation in the code is zero-shot classification (`main.py:test_zeroshot_3d_core`): pc embedding against the text embeddings of the 1,156 LVIS category prompts, Top-1/3/5, on OpenShape's `lvis_testset.txt`. "Cross-modal Retrieval" in the README is two qualitative figures, no code, no metric. UPSTREAM FACT. So the two baselines Kyzen asked about (SCA3D, Uni3D) each ship a protocol of their own kind -- caption->shape on ShapeNet, category-name classification on LVIS -- and neither is the seven-condition asset-level R@k of Table 1; the paper's "we followed the retrieval strategy explicitly described in their original papers" (rebuttal to cY7o) therefore cannot mean a shared query construction. Table 1's query source stays UNKNOWN.

**pc-only gallery probe (peer Claude's hypothesis, 2026-09-07 15:3x–16:0x).** A peer session (Kyzen's ULIP_RAG project) read MetaFind's Table 1 as "untrained equal-weight mean of frozen ULIP-2 unit vectors against a gallery holding the pc vector alone", from the near-midpoint fit of T+PC (44.5 vs (13.8+75.1)/2 = 44.4) and I+PC (45.8 vs 43.4). Checked: the fit fails for `full` (51.7 vs the three-way mean 33.5) and for every R@5 cell (T+PC 71.3 vs 50.6), so the fingerprint is two cells wide. Run anyway as a diagnostic: `tools/probes/exp_mean_pool_weak_trio.py --gallery pc --skip-resample --device cpu` (new flags; released ULIP-2, no training, val 4,569 vs 4,569, gallery = canonical pc vector only, which contradicts 2.4 and the rebuttal). R@1 %, `mean` pooling (`raw` in the JSON):

| query text / image / pc | text | image | pc | T+I | T+PC | I+PC | full |
|---|---|---|---|---|---|---|---|
| own attrs / own view / canonical | 50.6 | 70.4 | 100.0 | 73.8 | 99.8 | 99.8 | 99.7 |
| cat_size / own view / canonical | 16.8 | 70.4 | 100.0 | 61.3 | 99.9 | 99.8 | 99.7 |
| cat_size / thumbnail / canonical | 16.8 | 68.7 | 100.0 | 55.9 | 99.9 | 99.9 | 99.6 |
| BLIP caption / thumbnail / canonical | 31.3 | 68.7 | 100.0 | 60.1 | 99.9 | 99.9 | 99.6 |
