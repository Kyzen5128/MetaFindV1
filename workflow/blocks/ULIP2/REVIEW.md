...
## R-35 · 一族沉默錯誤的四個變體（2026-08-30）

**紀錄於此，因為提出者寫不了檔。** 變體 1-3 與這個分族的框架來自 INTEGRATOR
`metafindv1-e7 [ba9699]`，他全程 ON HOLD、不能寫檔；變體 4 是我在本日審查中量到的。
依 `paused-role-findings-die`：停工角色的發現必須交給寫得了檔的人，否則隨對話消失。

**四個變體共有的形狀：一個東西在系統裡是有效的，而系統沒有辦法回答「它從哪來」
或「它為什麼成立」。**

```
1  協定寫了，生效路徑不讀          現在就是錯的
2  指紋寫了，沒有消費者            現在就是錯的
3  產物存在，作者不明              現在就是缺的
4  性質成立，但沒有東西強制它       現在是對的      ← 最危險
```

### 為什麼第 4 個最危險（INTEGRATOR 的論證，我認可並驗證過實例）

**前三個被審查看到時是一個缺陷。第 4 個被審查看到時是一段正確的程式碼 —— 因為它確實正確。**

本日實例：`metafind/eval/run_retrieval.py` 的 `score_streaming`。
```
崩潰 gallery、d=1280、L2 正規化，tie_count 是否隨 `block` 改變
  ng=999 / 4569 / 9138    float32  6/6 會漂     float64  0/6
score_streaming 不檢查輸入 dtype（只驗 targets.shape 與欄位範圍）
```
**它今天跑出來的每個數字都對，通過測試、通過 Codex R5 APPROVED —— 因為 `encode_pools`
剛好回傳 float64。沒有任何工具能區分「因為被保證所以對」與「因為運氣好所以對」。**

**它不是等著被發現的錯，是等著呼叫端換人的正確。**

**要求的守衛今天一次都不會觸發。它的價值是把「碰巧」變成「保證」，
讓下一個呼叫者不必知道這件事。在它落地前，R2 MAJOR 不應標為結案 ——
Codex 核可的是現在的行為，不是它不會退回去。**

### 一個相關但不同的子形狀：合成時沒人檢查兩個部分是否同一時刻

本日兩例，一個是 INTEGRATOR 的、一個是我的：
```
INTEGRATOR  socket 現讀 + name/ref 沿用上一輪
            → 兩個欄位各自都曾為真，合成的身分從來沒存在過
本審查員     別人的原文摘要 + 我自己加的引號
            → 摘要為真、引號為真，合成的引文從來沒存在過
```
**INTEGRATOR 的補充值得逐字保留：「這兩種錯都不會產生任何不一致的痕跡。
一個過期的 ref 看起來就是一個 ref，一個被加了引號的摘要看起來就是一句引文。
錯誤不在內容裡，在內容之間的關係裡，而我們所有的檢查都只看內容。」**

### 待辦

- `score_streaming` 的 dtype 守衛與其具名測試（已向 ULIP2 Engineer `[943a73]` 提出）
- INTEGRATOR 的 `FINDINGS_20260827_CAPTURE.md` §3 仍是舊措辭，他不能改；需有人代寫或解除他的 hold
