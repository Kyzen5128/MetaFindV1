...
| **S-3** | Background matches upstream | mean corner luminance `= 0` | all regenerated renders |
| **S-4** | Framing matches upstream | longest side ÷ 224 = `0.60 ± 0.03` | ≥200 assets |
| **S-5** | **The correction is confirmed against an official upstream artifact** | `FIND-9`'s harness re-run: R@1 vs ULIP's `image_feat` **rises from 83.5% toward the point tower's 98.0%** | 200 assets, mismatched pairs as control, chance 0.5% |
