...
```
58637f3  "WIP UNACCEPTED: R-12 -- COLOR_0 withdrawn from the texture class"
         1 file changed: HANDOFF.md +96.  ZERO lines of code.

4e5053f  "docs: DL-010 -- upstream is a source, not a forbidden zone"
         metafind/data/pointclouds.py  +57   <- the modulate flag, the texture
                                                carve-out, SAMPLER_VERSION 5->6
         tests/test_pointclouds.py     +56
         workflow/CONTEXT.md           +27
         workflow/DECISION_LEDGER.md   +19
         workflow/roles/README.md      +90
         workflow/blocks/ULIP2/REVIEW.md +84
```

#### The mechanism, which matters more than the mislabelling

`4e5053f`, `e4ade5c` and `75abd02` were **not made by this block.** They carry Master's and the
Reviewer's content — `DECISION_LEDGER.md`, `CONTEXT.md`, `roles/README.md`, `REVIEW.md` — files
this block does not write.

**Every session shares one working tree and one git identity.** `git log --format="%an %ae"` shows
`Kyzen5128 <legend2341528@gmail.com>` on all six recent commits, so git cannot tell the roles
apart. When another session ran `git add -A`, it swept up this block's **uncommitted** edits to
`pointclouds.py` and `tests/test_pointclouds.py` and committed them under its own message.

**This is not a one-off.** Any two roles working concurrently will do it again, and the failure is
silent in both directions: the sweeper's commit gains changes it did not make, and the author's
commit loses the changes it announces.

#### Why it matters here specifically

- `experiments.md` §7 requires a result to be attributable to the code state that produced it.
  Anyone tracing *"when did the texture carve-out land"* opens `58637f3`, finds documentation only,
  and skips `4e5053f` because it is tagged `docs:`.
- `code-changes.md` §13 requires an inspectable diff. A research-critical **data-semantics** change
  is bundled with three governance files belonging to a different role.
- The 3.3-hour regeneration is about to produce a corpus whose provenance points at the wrong
  commit.

#### Correction of record — the authoritative mapping

| Change | Commit that actually contains it |
|---|---|
| `COLOR_0` withdrawn from `texture`; `modulate` flag; `SAMPLER_VERSION` 5→6 | **`4e5053f`** |
| `test_texture_bases_are_not_modulated_by_color0` | **`4e5053f`** |
| `R-12` finding, deviation #5, `CONTEXT.md` staleness report | `58637f3` |
| `P3` implementation, `SAMPLER_VERSION` 4→5 | `838b4b2` |
| `is_complete()` version gates, `SAMPLER_VERSION` 3→4, `U-W`, `U-X` | `a548623` |
| `n04` camera correction, `renderer_version` 2→3 | `2a8ded4` |
| `meshload`, frame correction, `COLOR_0` recovery | `bc863b7` |

**The corpus about to be produced was generated by the tree at `58637f3`, whose content is the sum
of all of the above.** History is not rewritten; the mapping is recorded.

#### ASK — Master's, because it is a cross-role process rule

The Reviewer's suggestion — separate code commits from governance commits — does not fix this on
its own. **The mechanism is `git add -A` against a shared tree**, and it will re-fire whenever two
roles are active. Options Master should route:

1. Every role commits by **explicit path**, never `git add -A`.
2. Roles are given distinct git identities, so `%an` can attribute a commit.
3. Roles work in separate worktrees.

**This block does not choose between them.** It will use explicit paths from now on regardless,
because that costs nothing and removes half the failure.

#### STATE

Behaviour is unaffected and independently verified — `R-13` PASS on all four conditions, n=37 and
n=60. **This is a provenance defect, not a correctness one.** It does not block the regeneration.
