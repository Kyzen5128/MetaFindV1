...
const NOTES = [
"各位好，今天報告 MetaFind 重現的進度。內容分六段：MetaFind 在做什麼、之前錯在哪、改了什麼與 Stage 1 的測試、融合分數為什麼還太高、Stage 2 做了什麼、接下來的計畫。所有數字都有出處，寫在 repo 的筆記裡。",
...
"大綱六段。前兩段是背景和診斷，中間兩段是測試與證據，最後兩段是 Stage 2 和計畫。",
...
"重點結論：七個新版本的 shape 全在 0.40 到 0.44。Fusion 一份或兩份、圖片平均或逐張、正規化開或關、視角固定或隨機，形狀都一樣。如果問題在架構，換架構形狀就會變。它沒變，所以問題不在架構，在資料。第四段證明這件事。",
"為什麼選 P1：它最乾淨，三個修正各有論文依據；單模態最接近論文；其他版本沒有更好；而且它可以被推翻，學習率、代數、query 觀測都在重跑。",
"第四段：融合分數為什麼還太高。這一段回答你上午的問題：是不是 ULIP-2 被改了？",
...
// ================================================================ TOC
{
  ...
  const items = [["01", "MetaFind 在做什麼", "任務、模型、怎麼訓練、論文的成績單長什麼樣"],
                 ["02", "之前錯在哪、原因", "拿答案對答案；文字太長；點雲數值太大；工具 bug"],
                 ["03", "改了什麼、Stage 1 做了哪些測試", "八個版本、目前數據、為什麼選 P1"],
                 ["04", "融合分數為什麼還太高", "ULIP-2 沒被動；query 跟 gallery 太像；實驗證明"],
                 ["05", "Stage 2 做了什麼", "佈局分支、四次跑的結果、候選 S2-C"],
                 ["06", "接下來計畫", "五段排程、判斷規則、要你決定的事"]];
  items.forEach((it, i) => {
    const col = i % 2, row = Math.floor(i / 2);
    const x = 0.6 + col * 4.7, y = 1.35 + row * 1.25;
...

{
  ...
  table(s, [["項目", "我們", "論文", "差異"],
            ...
            ["切分", "72 / 8 / 20：訓練 31,985、驗證 4,569、測試 9,138（封存不碰）", "80 / 20", "我們多切一塊驗證用"],
...
// ================================================================ 02
section("02", "之前錯在哪、原因是什麼", "第一次的分數太高。主因一個，其他六個。每一項都有證據。");
...
// ================================================================ 03
section("03", "改了什麼、Stage 1 做了哪些測試", "改法對應論文哪一句；八個版本一次只換一個東西；目前數據；為什麼選 P1。");
...
// ================================================================ 04
section("04", "融合分數為什麼還太高", "先排除 ULIP-2 被改；再用數字證明是 query 跟 gallery 太像；最後用實驗確認。");
...
// ================================================================ 05
section("05", "Stage 2 做了什麼", "房間層級微調：資料、配方、四次跑的結果、候選版本。");
...
// ================================================================ 06
section("06", "接下來計畫", "五段排程、每段的判斷規則、要你決定的事。");
