...
def test_the_degraded_render_exclusion_does_not_read_as_a_stale_gallery(capsys, monkeypatch):
    ...
    # Stop at the encoding boundary after checking the executed pools. The old
    # broad except accepted any failure and created real DataLoader workers
    # with a None backbone; their failed iterator was finalized in later tests.
    # A named sentinel proves this boundary, not an unrelated error, was reached.
    class ReachedEncoding(Exception):
        pass

    def inspect_pools(backbone, model, query_uids, gallery_uids, *args, **kwargs):
        assert query_uids == ["a", "c"]
        assert gallery_uids == ["a", "c"]
        raise ReachedEncoding

    monkeypatch.setattr(n15, "encode_pools", inspect_pools)
    with pytest.raises(ReachedEncoding):
        n15.run_protocol("filtered", proto, splits, None, None, "mean", "cpu",
                         4, "none", 0, 8, exclude_uids={"b"})

    # The executed pools are checked above; the exclusion must also remain
    # visible in the operator-facing output.
