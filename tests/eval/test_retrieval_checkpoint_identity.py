...
@pytest.mark.parametrize("field,value", [("gallery_source_status", "verified"),
                                        ("allow_legacy_gallery_index", False),
                                        ("use_layout", False),
                                        ("temperature_init", {"source": "stage2_recipe"}),
                                        ("semantic_source_status", "verified"),
                                        ("allow_legacy_semantic_inputs", False)])
@pytest.mark.parametrize("has_metadata", [True, False])
def test_legacy_stage2_sidecar_cannot_add_source_provenance(tmp_path, field, value, has_metadata):
    rp, rec = write_s2(tmp_path)
    if not has_metadata:
        torch.save({"trainable_state": {}}, rec["uri"])
        from pathlib import Path
        rec["sha256"] = hashlib.sha256(Path(rec["uri"]).read_bytes()).hexdigest()
        rp.write_text(json.dumps({"full": rec}))
    parent = {"sha256": "a" * 64}
    assert gi.load_stage2_checkpoint_record(rp, parent, allow_legacy=True) == rec
    rec[field] = value
    rp.write_text(json.dumps({"full": rec}))
    with pytest.raises(ValueError, match=f"unbound {field}"):
        gi.load_stage2_checkpoint_record(rp, parent, allow_legacy=True)
...
def test_load_fusion_tower_uses_checkpoint_flags_over_current_protocols(tmp_path, monkeypatch):
    from metafind.train import stage1
    ...
    restored = load_fusion_tower(p, "cpu")
