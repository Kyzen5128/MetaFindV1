...
def test_random_view_workers_change_the_arm_but_seed_remains_a_repeat():
    single = _training(_query_image_policy="random_view", _num_workers=0,
                       _preload=True)
    workers = {**single, "_num_workers": 4, "_preload": False}
    first, recipe = _arm(training=single)
    assert first != _arm(training=workers)[0]
    assert first == _arm(_values(seed=2), training=single)[0]
    assert recipe["query_image_rng"] == {
        "version": 1, "generator": "python.random", "num_workers": 0,
        "seed_source": "training_seed", "stream_lifetime": "run",
    }
    assert _arm(training=workers)[1]["query_image_rng"]["seed_source"] == (
        "torch_dataloader_worker_seed")
    # Cache residency by itself is not another treatment when workers match.
    assert first == _arm(training={**single, "_preload": False})[0]


@pytest.mark.parametrize("workers", [None, -1, True, "4"])
def test_random_view_refuses_an_unknown_or_invalid_effective_worker_count(workers):
    with pytest.raises(UnsupportedProtocol, match="random_view.*_num_workers"):
        _arm(training=_training(_query_image_policy="random_view",
                                _num_workers=workers))


def test_deterministic_mean_keeps_preloading_and_workers_out_of_the_arm():
    single = _training(_query_image_policy="same_mean", _num_workers=0,
                       _preload=True)
    workers = {**single, "_num_workers": 4, "_preload": False}
    assert _arm(training=single)[0] == _arm(training=workers)[0]
    assert "query_image_rng" not in _arm(training=single)[1]
...
OPTIONAL_TREATMENT_KEYS = {
    # Random view draws depend on the effective worker seed/lifecycle policy;
    # deterministic mean pooling retains the historical worker-independent arm.
    "query_image_rng",
