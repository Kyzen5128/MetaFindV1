...
@pytest.mark.parametrize("pinned", [None, 0.25])
def test_lambda_initialization_checkpoint_and_overlay_agree(tmp_path, use_layout, pinned):
    """A shared pinned-lambda recipe must still yield a replayable no-layout arm.

    Exercise the trainer's initializer and payload writer, strict metadata/byte
    loader and real query overlay. Full also retains its literal/ratio behavior;
    the ratio oracle uses two mean-fusion vectors with norms 5 and 12.
    """
    import copy
    import hashlib
    import json
    ...
    from metafind.train.gallery_index import load_stage2_checkpoint_record, stage2_layout_settings
    from metafind.train.stage2 import initialise_layout_lambda, stage2_checkpoint_payload

    fusion = FusionConfig(dim=4, kind="mean")
    layout = ESSGNNConfig(
        node_feat_dim=4, edge_feat_dim=4, out_dim=4, hidden_dim=4,
        n_layers=1, use_io_projections=True,
        architecture_family="appendix_shared_msg", pooling="normalised_sum")
    model = MetaFindDualTower(DualTowerConfig(
        dim=4, tower_sharing="shared_backbone_separate_fusion",
        query_fusion=fusion, gallery_fusion=fusion, use_layout=use_layout,
        essgnn=layout if use_layout else None, init_lambda=7.0))
    model.freeze_gallery(True)
    data = SimpleNamespace(asset_vectors={
        name: {modality: np.asarray(vector, dtype=np.float32)
               for modality in ("text", "image", "pc")}
        for name, vector in (("A", [3, 4, 0, 0]), ("B", [0, 12, 0, 0]))})
    sample_list = [("room", 0, "A"), ("room", 1, "B")]
    initialization = initialise_layout_lambda(
        model, {"init_lambda": pinned, "init_lambda_ratio": 0.1},
        sample_list if use_layout else None, {"room": {}} if use_layout else None,
        data if use_layout else None, "cpu", arch_protocol={"pooling": "normalised_sum"})
    if use_layout:
        expected = pinned if pinned is not None else 0.85
        assert initialization["init_lambda"] == pytest.approx(expected)
        assert model.query.layout_weight.item() == pytest.approx(expected)
        assert initialization["init_lambda_ratio"] == (None if pinned is not None else 0.1)
        if pinned is None:
            assert initialization["fused_query_norm_median"] == 8.5
    else:
        assert initialization is None
        assert model.query.layout_encoder is None and model.query.layout_weight is None

    state_path = tmp_path / "stage2.pt"
    record = {"uri": str(state_path), "variant_id": "full" if use_layout else "no_layout",
              "use_layout": use_layout, "lambda_init": initialization,
              "stage1_checkpoint_sha256": "a" * 64}
    torch.save(stage2_checkpoint_payload(model, MetaFindContrastiveLoss(), record), state_path)
    record["sha256"] = hashlib.sha256(state_path.read_bytes()).hexdigest()
    record_path = tmp_path / "record.json"
    record_path.write_text(json.dumps(record))
    restored = load_stage2_checkpoint_record(record_path, {"sha256": "a" * 64})
    restored_layout, restored_lambda = stage2_layout_settings(restored)
    assert restored_layout is use_layout
    if use_layout:
        assert restored_lambda == pytest.approx(expected)
    else:
        assert restored_lambda is None
    target = copy.deepcopy(model)
    with torch.no_grad():
        for parameter in target.query.parameters():
            parameter.add_(1.0)
    gallery_before = {key: value.clone() for key, value in target.gallery.state_dict().items()}
    overlay_stage2_weights(target, restored, "cpu")
    for key, value in model.query.state_dict().items():
        torch.testing.assert_close(target.query.state_dict()[key], value, rtol=0, atol=0)
    for key, value in gallery_before.items():
        torch.testing.assert_close(target.gallery.state_dict()[key], value, rtol=0, atol=0)
