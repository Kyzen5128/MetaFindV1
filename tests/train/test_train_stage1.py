...
@pytest.mark.parametrize("pool_size,limit,pack_coverage", [
    (0, None, False), (63, None, False), (128, 32, False),
    (128, None, True), (64, None, False)])
def test_main_requires_a_full_batch_before_loading_backbone(
        monkeypatch, tmp_path, pool_size, limit, pack_coverage):
    """Exercise the CLI preflight after split, observation coverage and limit."""
    import sys
    from metafind.models import ulip_backbone
    from metafind.train import stage1

    protocols(tmp_path, enc={"view_aggregation": {"n_views": 12}})
    use(monkeypatch, tmp_path)
    (tmp_path / "splits.json").write_text(json.dumps({
        "object": {"train": [f"asset-{i}" for i in range(pool_size)]}}))
    monkeypatch.setattr(stage1, "resolve_run_paths", lambda *args: None)
    monkeypatch.setattr(stage1.runlog, "runtime_source_status", lambda: "ok")

    class ReachedBackbone(Exception):
        pass

    def reached_backbone(*args, **kwargs):
        raise ReachedBackbone

    monkeypatch.setattr(ulip_backbone, "ULIPBackbone", reached_backbone)
    if pack_coverage:
        class PartialPack:
            path = tmp_path / "query_pack.json"
            arms = ("text",)
            sha256 = "a" * 64
            ...
            def covered(self, uids):
                return uids[:63], uids[63:]

        monkeypatch.setattr(stage1, "QueryPack", PartialPack)
    argv = ["stage1", "--phase", "final", "--device", "cpu",
            "--query-observation",
            "second_observation" if pack_coverage else "same_record"]
    if pack_coverage:
        argv += ["--query-pack", str(tmp_path / "query_pack.json")]
    if limit is not None:
        argv += ["--limit", str(limit)]
    monkeypatch.setattr(sys, "argv", argv)
    if pool_size == 64 and limit is None:
        with pytest.raises(ReachedBackbone):
            stage1.main()
    else:
        with pytest.raises(SystemExit, match="drop_last=True.*no training batches"):
            stage1.main()
...
def test_the_selection_is_deterministic_across_processes():
    ...
    runs = {subprocess.run([sys.executable, "-c", code], capture_output=True,
                           text=True, cwd=str(pathlib.Path(__file__).parents[2]),
