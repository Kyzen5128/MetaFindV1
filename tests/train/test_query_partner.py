...
def uids():
    ...
    if len(u) < 2:
        pytest.skip("need at least two dev_val assets for a partner comparison")
...
def test_query_side_reads_partner_text_and_image_but_own_pc(uids):
    ...
    with np.load(paths.EMBEDDINGS / f"{p}.npz") as cached:
        assert np.allclose(item["q_text"], cached["text"].astype(np.float32))
        views = cached["views"]
        assert np.allclose(item["q_image"], views[uid_seed(p) % len(views)].astype(np.float32))
    assert "q_pc" not in item                     # pc falls back to the asset's own
    assert not np.allclose(item["q_text"], item["text"])


def test_unknown_partner_policy_refused():
    from metafind.train.stage1 import Stage1Dataset
    with pytest.raises(ValueError, match="unknown partner policy"):
        Stage1Dataset(["fixture-a", "fixture-b"], "mean", partner="neighbour")
