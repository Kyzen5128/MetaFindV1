...
def inputs(tmp_path, monkeypatch):
    ...
    node_source = build_node_source(text, ENCODER)
    ...
    edge_file = tmp_path / "sem_edge_embeddings.npz"
    entries = {key: {"sentence": "A chair beside another chair.",
                     "embedding_uri": str(edge_file) + "#0"}}
    edge_source = build_edge_source(entries, ENCODER)
    np.savez(edge_file, keys=np.array([key]),
    ...
    write("sem_edge_cache.json", {
        ...
        "entries": entries, "source_identity": edge_source,
        "embedding_artifact": {"uri": str(edge_file),
                               "sha256": hashlib.sha256(edge_file.read_bytes()).hexdigest()}})
...
def test_stage2_uses_relation_cache_keys_and_preserves_node_vectors(inputs):
    ...
    assert data.semantic_source_status == {"node": "verified", "edge": "verified",
                                           "encoder_match": "verified"}


@pytest.mark.parametrize("source", ["node", "edge", "legacy_edge"])
def test_semantic_vectors_consume_the_bytes_read_before_path_replacement(inputs, monkeypatch, source):
    root, _, _, nodes = inputs
    target = root / ("nodes.npz" if source == "node" else "sem_edge_embeddings.npz")
    if source == "legacy_edge":
        cache_path = root / "sem_edge_cache.json"
        cache = json.loads(cache_path.read_text())
        cache.pop("source_identity")
        cache.pop("embedding_artifact")
        cache_path.write_text(json.dumps(cache))
        with np.load(target) as stored:
            arrays = {key: stored[key] for key in stored.files if key != "source_identity_sha256"}
    ...
    def replace_after_read(path):
        ...
        if path == target:
            calls.append(path)
            if len(calls) == 1:
                with np.load(io.BytesIO(raw)) as stored:
                    arrays = dict(stored)
                # Keep source digest/row IDs identical: source provenance alone
                # cannot notice that the vector file was replaced after hashing.
                arrays["embeddings"] = arrays["embeddings"] + 100
                np.savez(target, **arrays)
        return raw

    monkeypatch.setattr(Path, "read_bytes", replace_after_read)
    data = s2.Stage2Data("cpu", allow_legacy_semantic_inputs=source == "legacy_edge")
    assert calls == [target]
    np.testing.assert_array_equal(data.node_vectors["A"], nodes[0])
    np.testing.assert_array_equal(next(iter(data.sem_cache.values())), [1, 2, 3, 4])
    with np.load(target) as replaced:
        expected = nodes if source == "node" else np.array([[1, 2, 3, 4]])
        np.testing.assert_array_equal(replaced["embeddings"], expected + 100)
    assert data.semantic_source_status["edge"] == ("legacy_unbound" if source == "legacy_edge" else "verified")


def test_changed_node_text_is_refused_even_with_new_input_snapshot_and_same_relations(inputs):
    root, protocols, _, _ = inputs
    text_path = root / "procthor_object_text.json"
    text = json.loads(text_path.read_text())
    text["A"]["text"] = "Completely different newly reannotated asset: red metal chair."
    text_path.write_text(json.dumps(text))
    identity = s2.capture_stage2_input_identity(*protocols, ["h"])
    s2.verify_stage2_input_identity({"input_identity": identity}, current_inputs=True)
    with pytest.raises(ValueError, match="source text or membership changed"):
        s2.Stage2Data("cpu")


def test_semantic_node_and_edge_encoder_mismatch_is_refused(inputs):
    root, _, _, _ = inputs
    path = root / "sem_edge_cache.json"
    cache = json.loads(path.read_text())
    other = dict(ENCODER, state_sha256="4" * 64)
    source = build_edge_source(cache["entries"], other)
    cache["source_identity"] = source
    edge_path = root / "sem_edge_embeddings.npz"
    with np.load(edge_path) as old:
        arrays = dict(old)
    arrays["source_identity_sha256"] = np.array(source_identity_sha256(source))
    np.savez(edge_path, **arrays)
    cache["embedding_artifact"]["sha256"] = hashlib.sha256(edge_path.read_bytes()).hexdigest()
    path.write_text(json.dumps(cache))
    with pytest.raises(ValueError, match="different text encoders"):
        s2.Stage2Data("cpu", allow_legacy_semantic_inputs=True)


def test_legacy_semantic_inputs_need_explicit_opt_in_and_report_unbound(inputs):
    root, _, _, _ = inputs
    for record_name, array_path in (("procthor_node_embeddings.json", root / "nodes.npz"),
                                   ("sem_edge_cache.json", root / "sem_edge_embeddings.npz")):
        record_path = root / record_name
        record = json.loads(record_path.read_text())
        record.pop("source_identity")
        with np.load(array_path) as old:
            arrays = {k: old[k] for k in old.files if k != "source_identity_sha256"}
        np.savez(array_path, **arrays)
        if "sha256" in record:
            record["sha256"] = hashlib.sha256(array_path.read_bytes()).hexdigest()
        record.pop("embedding_artifact", None)
        record_path.write_text(json.dumps(record))
    with pytest.raises(ValueError, match="no bound source identity"):
        s2.Stage2Data("cpu")
    data = s2.Stage2Data("cpu", allow_legacy_semantic_inputs=True)
    assert data.semantic_source_status == {"node": "legacy_unbound", "edge": "legacy_unbound",
                                           "encoder_match": "legacy_unbound"}
    assert data.semantic_encoder_identity is None
