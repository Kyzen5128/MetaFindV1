...
def test_no_node_returns_non_zero_after_its_run_progress_block_closes():
    ...
    offenders = []
    source_files = sorted((paths.REPO / "metafind").rglob("*.py"))
    assert source_files, "the structural check must inspect the actual source tree"
        ...
        for fn in [n for n in ast.walk(tree)
            ...
            for ret in [n for n in ast.walk(fn) if isinstance(n, ast.Return)]:
                ...
                if src in ("0", "None", "progress.rc"):
                    continue
                if (path.relative_to(paths.REPO).as_posix(), src) in ALLOWED:
                    continue
