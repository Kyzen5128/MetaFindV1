...
from metafind.models.essgnn import ESSGCLShared, ESSGNN, ESSGNNConfig
...
def _appendix_numpy_mlp(value, parameters, name, *, trailing_silu=False):
    """Independent NumPy evaluation of the explicitly selected two-linear MLP.

    These activation/bias choices are IMPLEMENTATION CHOICES, not supplied by
    MetaFind's equations. No production module or PyTorch operation is called.
    """
    value = parameters[f"{name}.0.weight"] @ value + parameters[f"{name}.0.bias"]
    value = value / (1.0 + np.exp(-value))
    value = parameters[f"{name}.2.weight"] @ value
    if f"{name}.2.bias" in parameters:
        value = value + parameters[f"{name}.2.bias"]
    return value / (1.0 + np.exp(-value)) if trailing_silu else value


def _appendix_numpy_layer(h, x, edges, edge_features, parameters, structure):
    """Appendix Eq.10/13/14, looped over the declared directed neighbourhood.

    PAPER FACT: appendix.tex:32 builds m from OLD h/x and squared distance;
    :50 and :65 independently update x and h from that SAME message. D-14 /
    U-26 keep invariant semantic h and the appendix family. Sparse edge_index,
    scalar phi_x and MLP shapes are existing project choices, not new paper
    claims or decisions made by this oracle.
    """
    messages = []
    for edge, (receiver, neighbour) in enumerate(edges.T):
        distance_squared = sum((x[receiver, axis] - x[neighbour, axis]) ** 2
                               for axis in range(3))
        value = np.concatenate((h[receiver], h[neighbour], [distance_squared], edge_features[edge]))
        messages.append(_appendix_numpy_mlp(value, parameters, "phi_e",
                                            trailing_silu=structure == "egnn_appendix"))
    next_h, next_x = h.copy(), x.copy()
    for message, (receiver, neighbour) in zip(messages, edges.T):
        next_h[receiver] += _appendix_numpy_mlp(message, parameters, "phi_h")
        scalar = _appendix_numpy_mlp(message, parameters, "phi_x").item()
        for axis in range(3):
            next_x[receiver, axis] += (x[receiver, axis] - x[neighbour, axis]) * scalar
    return next_h, next_x


@pytest.mark.parametrize("structure", ["egnn_appendix", "linear_silu_linear"])
def test_appendix_layer_values_and_gradients_match_independent_numpy_oracle(structure):
    """One real layer: all h/x/e/parameter derivatives vs central differences.

    This isolates Eq.10/13/14. It does not establish trained-model quality,
    full-network pooling, or resolve the paper's competing main-text formulas.
    """
    # Degrees 3/1/2/0/0, asymmetric directions and edge attributes. Node 3 is a
    # neighbour only; node 4 is entirely isolated. A regular complete graph can
    # hide a mistaken aggregation axis, division by degree or symmetrization.
    arrays = {
        "h": np.array([[.2, -.3], [.7, .4], [-.5, .9], [.6, -.8], [1.1, .15]]),
        "x": np.array([[.2, -.1, .3], [1.1, .4, -.2], [-.4, .8, .7],
                       [.6, -.5, 1.2], [2., -1., .5]]),
        "e": np.array([[.1, -.2], [.3, .8], [-.6, .4], [.7, -.5], [.2, .9], [-.4, -.1]]),
    }
    edges = np.array([[0, 0, 0, 1, 2, 2], [1, 2, 3, 2, 0, 3]], dtype=np.int64)
    # Specify the oracle parameterization independently of production helpers;
    # strict loading also rejects an accidental architecture/parameter change.
    shapes = {}
    for name, input_width, output_width in (("phi_e", 7, 2), ("phi_h", 2, 2), ("phi_x", 2, 1)):
        shapes.update({f"{name}.0.weight": (2, input_width), f"{name}.0.bias": (2,),
                       f"{name}.2.weight": (output_width, 2)})
        if name != "phi_x" or structure == "linear_silu_linear":
            shapes[f"{name}.2.bias"] = (output_width,)
    rng = np.random.default_rng(20260908)
    parameters = {name: rng.uniform(-.4, .4, shape) for name, shape in shapes.items()}
    cfg = ESSGNNConfig(node_feat_dim=2, edge_feat_dim=2, out_dim=2, hidden_dim=2,
                      n_layers=1, use_io_projections=False, architecture_family=FAMILY,
                      h0_mode="semantic", distance="squared", coord_feat="current",
                      coords_agg="sum", normalize_coord_diff=False, mlp_structure=structure)
    layer = ESSGCLShared(cfg, edge_dim=2).to(dtype=DTYPE).eval()
    layer.load_state_dict({name: torch.tensor(value) for name, value in parameters.items()}, strict=True)
    tensors = {name: torch.tensor(value, requires_grad=True) for name, value in arrays.items()}
    actual_h, actual_x = layer(tensors["h"], tensors["x"], torch.tensor(edges), tensors["e"])
    expected_h, expected_x = _appendix_numpy_layer(arrays["h"], arrays["x"], edges,
                                                   arrays["e"], parameters, structure)
    np.testing.assert_allclose(actual_h.detach().numpy(), expected_h, rtol=1e-12, atol=1e-12)
    np.testing.assert_allclose(actual_x.detach().numpy(), expected_x, rtol=1e-12, atol=1e-12)
    np.testing.assert_array_equal(expected_h[4], arrays["h"][4])
    np.testing.assert_array_equal(expected_x[4], arrays["x"][4])

    # Non-vacuity: wrong directions, discarded e, or recomputing the coordinate
    # message with UPDATED h all produce observably different answers here.
    reversed_h, reversed_x = _appendix_numpy_layer(arrays["h"], arrays["x"], edges[::-1],
                                                  arrays["e"], parameters, structure)
    no_edge_h, no_edge_x = _appendix_numpy_layer(arrays["h"], arrays["x"], edges,
                                                np.zeros_like(arrays["e"]), parameters, structure)
    _, wrong_current_x = _appendix_numpy_layer(expected_h, arrays["x"], edges,
                                               arrays["e"], parameters, structure)
    assert not np.allclose(expected_h, reversed_h) and not np.allclose(expected_x, reversed_x)
    assert not np.allclose(expected_h, no_edge_h) and not np.allclose(expected_x, no_edge_x)
    assert not np.allclose(expected_x, wrong_current_x)

    # Signed, element-specific objectives observe both phi branches without a
    # norm-only loss that could hide a changed direction or coordinate update.
    h_weights = rng.uniform(-1., 1., expected_h.shape)
    x_weights = rng.uniform(-1., 1., expected_x.shape)
    loss_h = (actual_h * torch.tensor(h_weights)).sum()
    loss_x = (actual_x * torch.tensor(x_weights)).sum()
    named_parameters = dict(layer.named_parameters())
    grad_h = torch.autograd.grad(loss_h, tuple(named_parameters.values()),
                                 retain_graph=True, allow_unused=True)
    grad_x = torch.autograd.grad(loss_x, tuple(named_parameters.values()),
                                 retain_graph=True, allow_unused=True)
    for name, h_gradient, x_gradient in zip(named_parameters, grad_h, grad_x):
        if name.startswith("phi_e."):
            assert h_gradient is not None and h_gradient.abs().max() > 1e-8
            assert x_gradient is not None and x_gradient.abs().max() > 1e-8
        elif name.startswith("phi_h."):
            assert h_gradient is not None and x_gradient is None
        else:
            assert h_gradient is None and x_gradient is not None

    all_tensors = {**tensors, **named_parameters}
    gradients = dict(zip(all_tensors, torch.autograd.grad(
        loss_h + loss_x, tuple(all_tensors.values()))))

    def objective():
        h, x = _appendix_numpy_layer(arrays["h"], arrays["x"], edges,
                                     arrays["e"], parameters, structure)
        return float((h * h_weights).sum() + (x * x_weights).sum())

    epsilon = 1e-6
    for name, values in {**arrays, **parameters}.items():
        # Compare by variable name, independently of parameter registration order.
        actual_gradient = gradients[name].detach().numpy()
        ...
        np.testing.assert_allclose(actual_gradient, numerical, rtol=3e-6, atol=2e-8,
                                   err_msg=f"independent finite-difference gradient: {name}")
