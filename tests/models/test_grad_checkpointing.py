...
def _blocks(depth=3, dim=16, drop_path=0.5):
    ...
    for path in (str(_P(__file__).resolve().parents[2]), str(ulip_patch.ULIP_ROOT)):
...
def test_disabling_rng_preservation_changes_gradients():
    """The negative counterpart disables RNG restoration during recomputation.

    Both paths use `use_reentrant=False`; this checks `preserve_rng_state`,
    and does not compare reentrant versus non-reentrant checkpointing.
    """
    from torch.utils.checkpoint import checkpoint
    enc = _blocks()
    enc.train()
