...
def test_frozen_gallery_stays_deterministic_during_query_training():
    """Epoch-level model.train() must not reactivate gallery dropout."""
    torch.manual_seed(37)
    fusion = FusionConfig(dim=D, hidden=64, n_heads=4, n_layers=1, dropout=0.5)
    model = MetaFindDualTower(cfg(query_fusion=fusion, gallery_fusion=fusion,
                                  use_layout=False))
    model.freeze_gallery()
    assert model.train() is model
    assert model.query.training
    assert model.gallery_is_frozen()
    e = embeds()
    with torch.no_grad():
        assert torch.equal(model.gallery(e), model.gallery(e))
        assert not torch.equal(model.query(e), model.query(e))
    model.freeze_gallery(False)
    assert model.gallery.training
    with torch.no_grad():
        assert not torch.equal(model.gallery(e), model.gallery(e))


def test_unfreezing_respects_parent_eval_mode_until_training_resumes():
    model = MetaFindDualTower(cfg()).eval()
    model.freeze_gallery()
    model.freeze_gallery(False)
    assert not model.gallery_is_frozen()
    assert not model.gallery.training
    assert all(p.requires_grad for p in model.gallery.parameters())
    model.train()
    assert model.gallery.training


def test_separate_parameterless_fusions_are_not_tied_and_can_be_frozen():
    class Mean(torch.nn.Module):
        def forward(self, e):
            return torch.stack([e[m] for m in MODALITIES]).mean(0)

    model = MetaFindDualTower(cfg(use_layout=False))
    model.query.fusion = Mean()
    model.gallery.fusion = Mean()
    assert not model.fusion_is_tied()
    assert not model.gallery_is_frozen()
    model.freeze_gallery()
    model.train()
    assert model.gallery_is_frozen()
    assert model.query.training and not model.gallery.training
    assert torch.equal(model.gallery(embeds()), model.gallery(embeds()))
    model.freeze_gallery(False)
    assert model.gallery.training and not model.gallery_is_frozen()
    model.gallery.fusion = model.query.fusion
    assert model.fusion_is_tied()


def test_fully_shared_mean_cannot_freeze_only_the_gallery():
    fusion = FusionConfig(dim=D, kind="mean")
    model = MetaFindDualTower(cfg(tower_sharing="fully_shared", use_layout=False,
...
HP = dict(optimizer="adamw", learning_rate=1e-4, weight_decay=0.1,
          ...
          p_mask=0.30, decay_mask_tokens=False,
