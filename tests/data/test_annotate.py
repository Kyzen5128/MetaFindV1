...
def test_no_ulip_caption_fallback_exists():
    ...
    src = Path(__file__).resolve().parents[2] / "metafind" / "data" / "annotate.py"
    body = src.read_text()
...
def test_a_chinese_description_candidate_is_dropped_before_it_can_be_ranked(monkeypatch):
    ...
    monkeypatch.setattr(R, "rank_descriptions", fake_rank)
...
def test_a_clean_english_annotation_still_costs_one_structured_attempt(monkeypatch):
    """The language rule must not tax the records that were already fine."""
    from metafind.data import annotate_run as R

    monkeypatch.setattr(R, "rank_descriptions", lambda views, c: (
        c[0], [{"text": t, "clip_score": 0.3, "rank": i} for i, t in enumerate(c)]))
...
def test_the_repaired_record_is_marked_model_generated_not_human_repaired(monkeypatch):
...
def test_the_synset_table_covers_the_whole_lvis_vocabulary():
    ...
    meta = _json.loads(paths.LVIS_METADATA.read_text())
...
def test_one_failed_description_draw_does_not_cost_the_asset(monkeypatch):
