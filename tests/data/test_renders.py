...
def forbid_live_n04_render(monkeypatch):
    """These unit tests must explicitly supply PNGs at the Blender boundary.

    The retired pyrender visual tests below still render real geometry. The
    live n04 renderer requires OptiX and belongs outside this CPU test module.
    """
    from metafind.data import render_blender

    def forbidden(*args, **kwargs):
        raise AssertionError("live n04 render_asset is forbidden in CPU tests")

    monkeypatch.setattr(render_blender, "render_asset", forbidden)


@pytest.fixture
def channel_views(monkeypatch, forbid_live_n04_render):
    """Supply distinct full-size RGBA PNGs; keep process_one's checks real."""
    from metafind.data import render_blender

    _fake_views(monkeypatch, [
        _frame((200, 100, 50), 0.30, size=render_blender.RESOLUTION, seed=i)
        for i in range(render_blender.N_VIEWS)
    ])


def test_process_one_requires_an_explicit_render_boundary(tmp_path):
    """Forgetting the fake must fail before launching Blender or publishing."""
    from metafind.data.renders import is_complete

    out = tmp_path / "out"
    with pytest.raises(AssertionError, match="live n04 render_asset is forbidden"):
        process_one("guarded", _asset(tmp_path), out)
    assert not (out / "guarded.json").exists()
    assert not is_complete(out, "guarded")
...
def test_record_carries_the_channel_contract(tmp_path, channel_views):
    ...
    Synthetic PNGs exercise metadata assembly, hashing and channel shape. This
    does not verify that Blender drew the configured camera or denoiser.
    """
    from metafind.data import render_blender
    ...
    # Check the real provenance-reporting path, not synthetic pixel provenance.
...
def test_a_stale_sidecar_is_not_complete(tmp_path, channel_views):
