...
| 渲染引擎 | OpenShape 的 Blender 腳本（CYCLES、camera_dist 1.2、透明 RGBA 存檔、OptiX） | 沒寫 | 維持（UPSTREAM：ULIP-2 附錄把 3D 前處理交給 OpenShape） |
...
| 文字進 CLIP | 句型模板（v2_cm／attrs_v1） | P4 JSON、P6 `Platform Bed {size}` | **gallery 文字＝Figure 2 JSON 原字串**（欄位順序照 P4；`figure2_json` 模板已在 `resolve_stage1.py`）。CLIP 77 token 截斷照發生（量過：現有標註 100% 超過、平均 160 token、7.4% 連 description 都沒進去；換成一句短描述後會少）。這是 INFERENCE：作者展示的就是這個字串 |
| 影像→一條向量 | 12 張各過 ViT-bigG 取平均 | 沒寫 | 11 張取平均（DL-079 Q5 主線） |
...
| ProcTHOR 節點文字 | 類別名 "a counter top"（93 句） | "comprehensive semantic metadata"（P7，資料裡不存在） | **gemma 描述**（DL-077 Q10 甲，已核可未跑） |
| 場景圖邊 | support ∪ kNN(k=8) adjacency；語意邊 = gemma 一句話→凍結 CLIP 文字塔 | P8 | 維持（一致）；kNN 門檻是 IMPLEMENTATION CHOICE，重寫進紀錄 |
...
```
R1  渲染 v7        46,052 glb → 11 張/件；8 個 Blender 行程；~3 s/件 → 約 1.7～2 天      GPU
R2  標註 v10       gemma，11 張一次 prefill，Figure 2 十三欄；約 80 GPU 小時 → 約 3.3 天   GPU（R1 之後）
R3  點雲           不動
...
R8  Stage 2        9,600 屋、lr 5e-5、1 epoch、scene dropout 0.3、雙向；gallery 凍結        GPU 約 1 小時
...
2. 渲染器 v7 先煙霧測 3 件：確認 11 張、仰角 20°、透視、512 px、側檔記錄 `renderer_version: 7`；肉眼看圖。
