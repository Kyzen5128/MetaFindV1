...
| `n04_render_views` | **可執行** | `metafind/data/renders.py`。300 個資產、**隔離率 0.33%**（G3 門檻 2%）、837/min；11 張視圖全相異且無空白；9 條測試 |
| 其餘 **二十四個**節點 | **只有規格** | 無 |
...
122 個測試函式涵蓋六個模型模組、取樣器與渲染器（pytest 參數化後展開成 151 個 case），**沒有一條涵蓋任何節點的執行**。
