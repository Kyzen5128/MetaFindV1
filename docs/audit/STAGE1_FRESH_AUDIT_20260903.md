...
| C4 | **Figure 1：每個編碼器輸出 K 支向量（P1…PK, I1…IK, T1…TK）一起進 Fusion Layer** | 可能讀法：Fusion 吃的是**每個模態多個 token**（影像 = 11 張各一支、文字 = 多句或多 token、點雲 = 多支），而不是每模態一支平均向量 | Fusion 吃 **3 支**向量（text 1、image 1、pc 1；`fusion._stack` → `(B, 3, D)`） | 🟡 **重大疑點**。若 K = 視角數，論文的影像模態根本不需要「先平均」——11 張各自是一個 token，由 Transformer 自己融合。我們的 12 視角平均是自己加的一步。視角向量都在快取（`views (12,1280)`），**可以不重編碼就測「每視角一個 token」的 fusion** |
