...
OLD, ANNOTATION, STAGE2 = 2465870, 2043427, 2494552
...
receipt = HERE / 'stage1_deployment.json'
...
assert {pid: before[str(pid)]['start_ticks'] for pid in (OLD, ANNOTATION, STAGE2)} == {
    OLD: '76951292', ANNOTATION: '67087549', STAGE2: '77088621'}
...
assert old_bytes == subprocess.check_output(['git', 'show', 'HEAD:tools/chain_paper_stage1.sh'], cwd=ROOT)
new_bytes = SOURCE.read_bytes()
assert sha(new_bytes) == '77c8a588371e8d32bf2b8e88a79451f16b01bff46c114e46e7237de1c77fb8fb'
...
backup = HERE / 'chain_paper_stage1_before.sh'
with backup.open('xb') as stream:
    stream.write(old_bytes)
candidate = LIVE.with_name(LIVE.name + '.validated-20260908.new')
...
try:
    ...
    with LIVE.with_suffix('.log').open('ab', buffering=0) as log:
        log.write(b'\n=== REVIEWED WAITING STAGE1 RESTART 2026-09-08: manual exclusion schema fix\n')
