...
$$m_{ij} = \phi_e \left( h_i^l, h_j^l, \|x_i^l - x_j^l\|^2, a_{ij} 
ight) \quad (3)$$
...
where $e_{ij} pprox \phi_{inf}(m_{ij})$ represents a soft estimation of edge existence [18]. $\phi_{inf} : \mathbb{R}^{n_f} 	o [0, 1]^1$ is approximated by a linear layer followed by a sigmoid function [18]. This preserves the $E(n)$ properties because $m_{ij}$ is already $E(n)$ invariant [18].
...
$$\hat{A}_{ij} = g_e(z_i, z_j) = rac{1}{1 + \exp(w \|z_i - z_j\|^2 + b)} \quad (9)$$
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| $lpha$ | $	ext{bohr}^3$ | 0.092 | 0.235 | 0.085 | 0.088 | 0.084 | **0.044** | 0.223 | 0.142 | 0.071 |
...
$$m'_{ij} = \phi_e \left( h_i^l, h_j^l, \|Qx_i^l + g - (Qx_j^l + g)\|^2, a_{ij} 
ight) = \phi_e \left( h_i^l, h_j^l, \|x_i^l - x_j^l\|^2, a_{ij} 
ight) = m_{ij} \quad (12)$$
