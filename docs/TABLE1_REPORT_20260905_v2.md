...
> **2026-09-07 更正：** 本檔保留歷史分數；下列有限實驗不支持融合必定加分、作者觀測一定較弱或訓練一定不足等因果結論。現行自訂評估見 [CUSTOM_TABLE1_EVALUATION.md](CUSTOM_TABLE1_EVALUATION.md)，本次未重跑表格。
...
- 在表列的一個配置中，文字單獨 12.0，T+PC 95.4；這是 OBSERVED DATA，不能推出其他同物件文字或 fusion 都必定加分。Fusion 如何使用模態的因果機制仍需獨立診斷。
...
**更正（UNKNOWN）：** 較弱的 query 觀測與不同的融合訓練是候選假說，這些表格不足以認定作者同時使用了兩者。固定模型、候選池和觀測的敏感度實驗仍可繼續；不得用調整到較低分數當作還原作者設定的證據。
