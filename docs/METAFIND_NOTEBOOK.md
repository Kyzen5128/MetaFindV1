...
| 4 | Stage 1 真正 retrain arms | 🔶 **跑中（Kyzen 2026-09-03「全程交給你處理」）**：P1 = attrs_v1 文字 + single_view 查詢 + pre-fusion L2（overlay `metafind_data_attrs`，10 輪）；P4 = P1 + 一份共用 Fusion（`metafind_data_attrs_shared`）；P3 = P1 + 12 視角當 12 個 token 進 Fusion（`metafind_data_attrs_tokens`）；**P5 = 三個模態的 query 都是第二份觀測**：文字改 `desc_v1`（自由描述一句），query 用第二名候選描述、點雲重取樣（query pack）、影像單一視角（`metafind_data_desc`）。依序排隊。P1 前三輪已顯示：只換影像不夠，含 text 或 pc 的條件仍往 100 爬（第 2 輪 T+PC 96.6、full 98.3），E1 的邏輯對每個模態各自成立，P5 因此而生。P1 第 0 輪 dev_val（4,569）：text 11.4 / image 17.8 / pc 45.8 / T+I 29.4 / T+PC 78.9 / I+PC 63.8 / full 82.1 —— 第一次落在論文的量級 |
