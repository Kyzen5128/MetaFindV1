...
Source : /home/kyzen/egnn/models/egnn_clean/egnn_clean.py (commit e9ca6c0)
...
Why vendored rather than imported from the clone:

    Both /home/kyzen/ULIP and /home/kyzen/egnn ship a top-level package named
    `models`. ULIP's has no __init__.py (namespace package) while egnn's does
    (regular package), so once egnn's `models` is resolved, Python prefers it
    and `models.pointbert` becomes unreachable regardless of sys.path order.
    MetaFind needs both repos at once, so the collision is structural, not a
    test artefact.

    This file depends only on torch, so vendoring costs nothing. ESSGNN extends
